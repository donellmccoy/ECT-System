...
{
    /// <summary>
    /// Data access object for managing <see cref="LineOfDuty"/> entities.
    /// Provides Line of Duty operations including access control, workflow management, and pending case counts for AFRC (Air Force Reserve Command).
    /// </summary>
    public class LineOfDutyDao : AbstractNHibernateDao<LineOfDuty, int>, ILineOfDutyDao
    {
        /// <summary>
        ...
        /// <summary>
        /// Determines the access level a user has for a specific LOD V3 case.
        /// </summary>
        /// <param name="userId">The user ID.</param>
        /// <param name="refId">The reference ID of the LOD case.</param>
        /// <returns>The access level (None, ReadOnly, or ReadWrite).</returns>
        public PageAccess.AccessLevel GetUserAccess_LOD_V3(int userId, int refId)
        {
            SqlDataStore store = new SqlDataStore();
            int access = (int)store.ExecuteScalar("lod_sp_UserHasAccess", userId, refId, (int)ModuleType.LOD);
        ...
        /// <summary>
        /// Saves a Line of Duty entity with validation.
        /// </summary>
        /// <param name="entity">The <see cref="LineOfDuty"/> entity to save.</param>
        /// <returns>The saved entity.</returns>
        /// <exception cref="PreconditionException">Thrown when required fields are missing or invalid.</exception>
        public override LineOfDuty Save(LineOfDuty entity)
        {
