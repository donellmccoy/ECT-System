...
namespace ALOD.Tests.Unit.Data
{
    [Trait("Category", "Unit")]
    [Trait("Layer", "Data")]
    public class RulesTypeDaoTests
    {
        private readonly Mock<IRuleTypeDao> _mockDao;

        public RulesTypeDaoTests()
        {
            _mockDao = new Mock<IRuleTypeDao>();
        }
        ...
        {
            // Arrange
            var entity = new RuleType();
            _mockDao.Setup(x => x.Save(It.IsAny<RuleType>())).Returns(entity);
            ...
            _mockDao.Verify(x => x.Save(It.IsAny<RuleType>()), Times.Once);
        }
        ...
        [InlineData(10)]
        [InlineData(50)]
        public void GetById_WithVariousIds_ReturnsRuleType(int id)
        {
            // Arrange
            _mockDao.Setup(x => x.GetById(id)).Returns(new RuleType());
        ...
        public void GetAll_ReturnsAllEntities()
        {
            ...
            _mockDao.Setup(x => x.GetAll()).Returns(ruleTypes);
        ...
        public void Delete_WithValidEntity_CallsDelete()
        {
            // Arrange
            var entity = new RuleType();
            _mockDao.Setup(x => x.Delete(It.IsAny<RuleType>()));

            // Act
            _mockDao.Object.Delete(entity);

            // Assert
            _mockDao.Verify(x => x.Delete(It.IsAny<RuleType>()), Times.Once);
        }
        ...
        public void SaveOrUpdate_WithValidEntity_ReturnsSavedEntity()
        {
            // Arrange
            var entity = new RuleType();
            _mockDao.Setup(x => x.SaveOrUpdate(It.IsAny<RuleType>())).Returns(entity);
            ...
            // Assert
            Assert.NotNull(result);
            _mockDao.Verify(x => x.SaveOrUpdate(It.IsAny<RuleType>()), Times.Once);
        }
