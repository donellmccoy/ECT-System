...
Namespace Web.Special_Case.NE
    ...
    Partial Class Secure_ne_init
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            Dim userAccess As PageAccess.AccessLevel = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCaseNE)

            Utility.VerifyUserAccess(userAccess, Resources.Messages.ERROR_NO_ACCESS, Resources._Global.StartPage)

            Dim scNE As SC_NE = MasterPage.LoadCase(refId)

            Session("refId") = refId

            Utility.UpdateCaseLock(userAccess, refId, ModuleType.SpecCaseNE)

            MasterPage.InitPageAccess(scNE)

            Dim startPage As String = MasterPage.GetStartPageTitle(scNE.Status)

            'remove the scNE from the rDao so no changes are persisted back
            SCDao.Evict(scNE)
