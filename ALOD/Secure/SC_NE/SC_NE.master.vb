...
Namespace Web.Special_Case.NE
    ...
    Partial Class SC_NEMaster
        Inherits System.Web.UI.MasterPage

        Private _dao As ISpecialCaseDAO
        Private _sc As SC_NE = Nothing
        ...
        Public Function ConstructSeniorMedicalReviewerTabVisibilityArgs() As SeniorMedicalReviewerTabVisibilityArgs
            ...
            workStatuses.Add(SpecCaseNEWorkStatus.SeniorMedicalReview)

            args.RefId = SpecCase.Id
            args.ModuleId = SpecCase.moduleId
            args.WorkStatusIds = workStatuses
            args.TabTitle = "NE Senior Med"
            args.Steps = Navigator.Steps

            Return args
        End Function

        Public Function GetStartPageTitle(ByVal workStatus As SpecCaseNEWorkStatus) As String
            Dim startPage As String = "NE Member"

            Select Case workStatus
                Case SpecCaseNEWorkStatus.InitiateCase
                    If (GroupId = UserGroups.MedicalTechnician) Then
                        startPage = "NE Med Tech"
                    End If

                Case SpecCaseNEWorkStatus.MedicalReview
                    If (GroupId = UserGroups.BoardMedical) Then
                        startPage = "NE Board Med"
                    End If
        ...
        Public Sub InitPageAccess(ByVal specCase As SC_NE)
        ...
        Public Function LoadCase(ByVal refId As Integer) As SC_NE
            Dim scNE As SC_NE = Nothing
            ...
            If (scNE Is Nothing) Then
                'we failed to load the case, nothing we can do
                ALOD.Logging.LogManager.LogError("NULL Non Emergent Surgery Request Error loading Non Emergent Surgery Request with requestId: " + refId.ToString())
                SetErrorMessage(Resources.Messages.ERROR_LOADING_CASE)
                Response.Redirect(Resources._Global.StartPage, True)
            End If
        ...
        Private Sub InitNavigator(ByVal refId As Integer)
            ...
            Dim scNE As New SC_NE

            Session("refId") = refId

            userAccess = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCaseNE)

            Utility.VerifyUserAccess(userAccess, Resources.Messages.ERROR_NO_ACCESS, Resources._Global.StartPage)

            scNE = LoadCase(refId)

            Utility.UpdateCaseLock(userAccess, refId, ModuleType.SpecCaseNE)
            InitPageAccess(scNE)
