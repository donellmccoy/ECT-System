...
Namespace Web.Special_Case.CMAS
    ...
    Partial Class SC_CMASMaster
        Inherits System.Web.UI.MasterPage

        Private _dao As ISpecialCaseDAO
        Private _sc As SC_CMAS = Nothing
        ...
        Public Function GetStartPageTitle(ByVal workStatus As SpecCaseCMASWorkStatus) As String
            Dim startPage As String = "CM Member"

            Select Case workStatus
                Case SpecCaseCMASWorkStatus.InitiateCase
                    If (GroupId = UserGroups.AFRCHQTechnician) Then
                        startPage = "CM HQ AFRC Tech"
                    End If

                Case SpecCaseCMASWorkStatus.Complete
                    If (GroupId = UserGroups.AFRCHQTechnician) Then
                        startPage = "CM HQ AFRC Tech"
                    End If
        ...
        Public Sub InitPageAccess(ByVal specCase As SC_CMAS)
        ...
        Public Function LoadCase(ByVal refId As Integer) As SC_CMAS
            Dim scCI As SC_CMAS = Nothing
            ...
            If (scCI Is Nothing) Then
                'we failed to load the case, nothing we can do
                ALOD.Logging.LogManager.LogError("NULL SpecCaseCMAS Error loading Case of Type (" + "SpecCaseCMAS" + ") with refId: " + refId.ToString())
                SetErrorMessage(Resources.Messages.ERROR_LOADING_CASE)
                Response.Redirect(Resources._Global.StartPage, True)
            End If
        ...
        Private Sub InitNavigator(ByVal refId As Integer)
            ...
            Dim scRS As New SC_CMAS

            Session("refId") = refId

            userAccess = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCaseCMAS)

            Utility.VerifyUserAccess(userAccess, Resources.Messages.ERROR_NO_ACCESS, Resources._Global.StartPage)

            scRS = LoadCase(refId)

            Utility.UpdateCaseLock(userAccess, refId, ModuleType.SpecCaseCMAS)
            InitPageAccess(scRS)
