...
Namespace Web.Special_Case.PH
    ...
    Partial Class Secure_sc_ph_Documents
...
#Region "Page Methods"
        ...
        Private Sub GetPHForm()
            ' Sets the url to get Form "snapshot" if workflow is completed
            If (SpecCase.WorkflowStatus.StatusCodeType.IsFinal) Then
                Dim _Documents = DocumentDao.GetDocumentsByGroupId(SpecCase.DocumentGroupId)
                ViewDocLink348.Attributes.Add("onclick", "" & ViewForms.PHFormPDFLinkAttribute(SpecCase, _Documents) & "")
            Else
                ViewDocLink348.Attributes.Add("onclick", "printForms('" & RequestId.ToString() & "', 'SC_PH');")
            End If
        End Sub
