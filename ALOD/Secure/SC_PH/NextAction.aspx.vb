...
Namespace Web.Special_Case.PH
    ...
    Partial Class Secure_sc_ph_NextAction
...
#Region "Fields..."
        ...
        Private boardStages As Short() = New Short() {
            CShort(SpecCasePHStatusCode.AFRCReview),
            CShort(SpecCasePHStatusCode.DelinquentAFRCReview)}
        ...
        Private unitStages As Short() = New Short() {
            CShort(SpecCasePHStatusCode.InitiateCase),
            CShort(SpecCasePHStatusCode.Delinquent)}
...
#Region "Page Methods..."
        ...
        Public Sub AddEmails(ByRef emails As MailManager, emailService As EmailService, action As WorkflowOptionAction, statusOut As Integer)
            ...
            If (statusOut = SpecCasePHWorkStatus.Complete) Then
        ...
        Public Function ChangeStatus(optionId As Integer, newStatus As Integer, text As String, comments As String) As Boolean
            ...
            If (newStatus <> SpecCase.Status) Then
                ...
                If (IsTimeToStoreForm()) Then
                    Dim docId As Long = StorePHForm()

                    LogManager.LogAction(ModuleType.SpecCasePH, UserAction.AddedDocument, SpecCase.Id, "Generated PH Form PDF with an ID of " & docId.ToString())
                End If
        ...
        Public Sub InsertRWOA(ByVal statusIn As Integer, ByVal statusOut As Integer)
            ...
            If (IsReturnedToUnit(statusCodeIn, statusCodeOut)) Then
                ...
                If GetSentToInfo(statusCodeOut).Item1 <> 0 Then
                    ...
                    SpecCase.ReturnToGroup = GetSentToInfo(statusCodeOut).Item1
                End If
        ...
        Protected Function GetSentToInfo(ByVal statusCodeOut As Integer) As Tuple(Of Integer, String)
            ...
            Select Case statusCodeOut
                Case SpecCasePHStatusCode.InitiateCase, SpecCasePHStatusCode.Delinquent
                    sentTo = "Unit PH"
                    sentToId = UserGroups.UnitPH
                Case SpecCasePHStatusCode.AFRCReview, SpecCasePHStatusCode.DelinquentAFRCReview
                    sentTo = "HQ AFRC DPH"
                    sentToId = UserGroups.HQAFRCDPH
                Case Else
                    sentTo = ""
            End Select
        ...
        Protected Function IsTimeToStoreForm() As Boolean
            If ((SpecCase.Status = SpecCasePHWorkStatus.Complete OrElse SpecCase.Status = SpecCasePHWorkStatus.Cancelled) AndAlso SpecCase.DocumentGroupId.HasValue) Then
                Return True
            End If
        ...
        Protected Function StorePHForm() As Long
            Dim doc As PDFDocument = New PDFDocument()
            doc.SetRenderingEngine(EngineType.Gecko)
            doc.AddWebPage(Server, "~/Secure/SC_PH/PrintableForm.aspx?refId=" & RefId, 1)
        ...
        Protected Sub SubmitButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
            ...
            If (ChangeStatus(workOption.Id, workOption.wsStatusOut, LogActionDisplayText, Server.HtmlEncode(CommentsTextBox.Text.Trim))) Then
                SetFeedbackMessage("Case " + SpecCase.CaseId + " successfully signed.  Action applied: " + LogActionDisplayText)
        ...
        Private Sub ShowRLB()
            Dim access As ALOD.Core.Domain.Workflow.PageAccessType = SectionList(PHSectionNames.PH_RLB.ToString())
