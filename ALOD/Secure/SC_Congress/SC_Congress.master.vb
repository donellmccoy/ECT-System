...
Namespace Web.Special_Case.CI
    ...
    Partial Class SC_CongressMaster
        Inherits System.Web.UI.MasterPage

        Private _dao As ISpecialCaseDAO
        Private _sc As SC_Congress = Nothing
        ...
        Public Function GetStartPageTitle(ByVal workStatus As SpecCaseCongressWorkStatus) As String
            Dim startPage As String = "CI Member"

            Select Case workStatus
                Case SpecCaseCongressWorkStatus.InitiateCase
                    If (GroupId = UserGroups.AFRCHQTechnician) Then
                        startPage = "CI HQ AFRC Tech"
                    End If

                Case SpecCaseCongressWorkStatus.Complete
                    If (GroupId = UserGroups.AFRCHQTechnician) Then
                        startPage = "CI HQ AFRC Tech"
                    End If
        ...
        Public Sub InitPageAccess(ByVal specCase As SC_Congress)
        ...
        Public Function LoadCase(ByVal refId As Integer) As SC_Congress
            Dim scCI As SC_Congress = Nothing
            ...
            If (scCI Is Nothing) Then
                'we failed to load the case, nothing we can do
                ALOD.Logging.LogManager.LogError("NULL SpecCaseCongress Error loading Case of Type (" + "SpecCaseCongress" + ") with refId: " + refId.ToString())
                SetErrorMessage(Resources.Messages.ERROR_LOADING_CASE)
                Response.Redirect(Resources._Global.StartPage, True)
            End If
        ...
        Private Sub InitNavigator(ByVal refId As Integer)
            ...
            userAccess = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCaseCongress)

            Utility.VerifyUserAccess(userAccess, Resources.Messages.ERROR_NO_ACCESS, Resources._Global.StartPage)

            sc = LoadCase(refId)

            Utility.UpdateCaseLock(userAccess, refId, ModuleType.SpecCaseCongress)
            InitPageAccess(sc)
