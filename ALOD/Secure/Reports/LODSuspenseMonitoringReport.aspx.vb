...
Namespace Web.Reports
    ...
    Public Class LODSuspenseMonitoringReport
        ...
        Private _report As ALOD.Core.Domain.Reports.LODSuspenseMonitoringReport
        ...
        Protected ReadOnly Property Report As ALOD.Core.Domain.Reports.LODSuspenseMonitoringReport
            Get
                If (_report Is Nothing) Then
                    _report = New ALOD.Core.Domain.Reports.LODSuspenseMonitoringReport(New NHibernateDaoFactory())
                End If
        ...
        Protected Function ConstructReportArguments(ByVal unitId As Integer, ByVal groupByChildUnits As Boolean) As LODSuspenseMonitoringReportArgs
            Dim args As LODSuspenseMonitoringReportArgs = New LODSuspenseMonitoringReportArgs()
        ...
        Protected Sub RunReport()
            ...
            If (Not UserHasPermission("LODSuspenseMonitoringReport")) Then
...
#Region "UI Procedures..."
        ...
        Protected Sub ConfigureDaysLabel(ByVal lbl As Label, ByVal value As Nullable(Of Integer), ByVal thresholdStatus As ProcessingTimeThresholdStatus)
            ...
            lbl.Text = Math.Round(value.Value, MidpointRounding.AwayFromZero)
        ...
        Protected Sub ConfigureDaysOpenLabel(ByVal lbl As Label, ByVal resultItem As LODSuspenseMonitoringReportResultItem)
            If (resultItem.DaysOpen = 0 AndAlso resultItem.IsTimeBeingProcessed = False) Then
                lbl.Text = "*"
            Else
                lbl.Text = Math.Round(resultItem.DaysOpen, MidpointRounding.AwayFromZero)
            End If
        ...
        Protected Sub gdvResults_DataBound(sender As Object, e As EventArgs) Handles gdvResults.DataBound
            ...
            If (Report.ResultsDataKey.Equals("UnitId")) Then
                ...
                gdvResults.Columns(4).Visible = False     ' Status
                gdvResults.Columns(21).Visible = False    ' Member Unit
                gdvResults.Columns(22).Visible = False    ' Member Name
            Else
                gdvResults.Columns(0).Visible = False     ' Unit
                gdvResults.Columns(1).Visible = True      ' Case Id
                gdvResults.Columns(4).Visible = True      ' Status
                gdvResults.Columns(21).Visible = True     ' Member Unit
                gdvResults.Columns(22).Visible = True     ' Member Name
            End If
        End Sub
        ...
        Protected Sub gdvResults_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gdvResults.RowDataBound
            ...
            If e.Row.RowIndex <> gdvResults.EditIndex Then
                ...
                Dim resultItem As LODSuspenseMonitoringReportResultItem = Nothing
                ...
                Else
                    ...
                    CType(e.Row.FindControl("lblStatus"), Label).Text = resultItem.CurrentStatus
                ...
                ConfigureDaysLabel(CType(e.Row.FindControl("lblDaysInStatus"), Label), resultItem.DaysInCurrentStatus, resultItem.DaysInCurrentStatusStatus)
        ...
        Protected Sub ResultsToBrowser()
            ...
            gdvResults.DataSource = Report.Results.OrderByDescending(Function(x) x.DaysOpen).ThenByDescending(Function(z) z.IsTimeBeingProcessed).ThenBy(Function(y) y.CaseId)
...
#Region "Excel Procedures..."
        ...
        Protected Sub ResultsToExcel()
            ...
            Response.Clear()
            Response.AddHeader("content-disposition", "attachment;filename=LOD_PM_Monthly_Suspense_Monitoring_Report_" + Date.Now.ToString("yyyyMMdd") + ".xls")
            Response.Charset = ""
...
#Region "PDF Procedures..."
        ...
        Protected Sub ResultsToPDF()
            ...
            For ct As Integer = 1 To pdf.PageCount
                ...
                'table header
                'pdf.AddLine(10, 545, 782, 545)

                ' Asterisk messages...
                If (ct = pdf.PageCount) Then
                    pdf.HPos = 0.0
                    pdf.Rect.SetRect(20, 31, 750, 31)
            ...
            Response.AddHeader("content-disposition", "attachment;filename=LOD_PM_Monthly_Suspense_Monitoring_Report_" + Date.Now.ToString("yyyyMMdd") + ".pdf")
...
#Region "XML Procedures"
        ...
        Private Sub ResultsToXml()
            ...
            Response.Clear()
            Response.AddHeader("content-disposition", "attachment;filename=LOD_PM_Monthly_Suspense_Monitoring_Report_" + Date.Now.ToString("yyyyMMdd") + ".xml")
            Response.Charset = ""
...
#Region "CSV Procedures"
        ...
        Private Sub ResultsToCSV()
            ...
            Response.Clear()
            Response.AddHeader("content-disposition", "attachment;filename=LOD_PM_Monthly_Suspense_Monitoring_Report_" + Date.Now.ToString("yyyyMMdd") + ".csv")
            Response.Charset = ""
