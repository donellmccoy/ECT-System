...
Namespace Web.Special_Case.MEB
    ...
    Partial Class SC_MEBMaster
        Inherits System.Web.UI.MasterPage

        Private _dao As ISpecialCaseDAO
        Private _sc As SC_MEB = Nothing
        ...
        Public Function ConstructSeniorMedicalReviewerTabVisibilityArgs() As SeniorMedicalReviewerTabVisibilityArgs
            ...
            workStatuses.Add(SpecCaseMEBWorkStatus.SeniorMedicalReview)
            workStatuses.Add(SpecCaseMEBWorkStatus.SeniorMedicalReviewAFPC)
            workStatuses.Add(SpecCaseMEBWorkStatus.SeniorMedicalReviewIPEB)
            workStatuses.Add(SpecCaseMEBWorkStatus.SeniorMedicalReviewFPEB)
            workStatuses.Add(SpecCaseMEBWorkStatus.SeniorMedicalReviewTransitionPhase)

            args.RefId = SpecCase.Id
            args.ModuleId = SpecCase.moduleId
            args.WorkStatusIds = workStatuses
            args.TabTitle = "MB Senior Med"
            args.Steps = Navigator.Steps

            Return args
        End Function

        Public Function GetStartPageTitle(ByVal workStatus As SpecCaseMEBWorkStatus) As String
            Dim startPage As String = "MB Member"

            Select Case workStatus
                Case SpecCaseMEBWorkStatus.InitiateCase
                    If (GroupId = UserGroups.AFRCHQTechnician) Then
                        startPage = "MB HQ AFRC Tech"
                    End If

                Case SpecCaseMEBWorkStatus.RWOAAction
                    If (GroupId = UserGroups.AFRCHQTechnician) Then
                        startPage = "MB HQ AFRC Tech"
                    End If

                Case SpecCaseMEBWorkStatus.AwaitingAFPC
                    If (GroupId = UserGroups.AFRCHQTechnician) Then
                        startPage = "MB HQ AFRC Tech"
                    End If

                Case SpecCaseMEBWorkStatus.MedicalReview
                    If (GroupId = UserGroups.BoardMedical) Then
                        startPage = "MB Med Off"
                    End If
        ...
        Public Sub InitPageAccess(ByVal specCase As SC_MEB)
        ...
        Public Function LoadCase(ByVal refId As Integer) As SC_MEB
            Dim scMEB As SC_MEB = Nothing
            ...
            If (scMEB Is Nothing) Then
                'we failed to load the case, nothing we can do
                ALOD.Logging.LogManager.LogError("NULL MEB Error loading MEB with requestId: " + refId.ToString())
                SetErrorMessage(Resources.Messages.ERROR_LOADING_CASE)
                Response.Redirect(Resources._Global.StartPage, True)
            End If
        ...
        Private Sub InitNavigator(ByVal refId As Integer)
            ...
            userAccess = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCaseMEB)
