...
Namespace Web.Special_Case.PW
    ...
    Partial Class SC_PWaiverMaster
        Inherits System.Web.UI.MasterPage

        Private _dao As ISpecialCaseDAO
        Private _sc As SC_PWaivers = Nothing
        ...
        Public Function ConstructSeniorMedicalReviewerTabVisibilityArgs() As SeniorMedicalReviewerTabVisibilityArgs
            ...
            workStatuses.Add(SpecCasePWWorkStatus.SeniorMedicalReview)

            args.RefId = SpecCase.Id
            args.ModuleId = SpecCase.moduleId
            args.WorkStatusIds = workStatuses
            args.TabTitle = "PW Senior Med"
            args.Steps = Navigator.Steps

            Return args
        End Function

        Public Function GetStartPageTitle(ByVal workStatus As SpecCasePWWorkStatus) As String
            Dim startPage As String = "PW Member"

            Select Case workStatus
                Case SpecCasePWWorkStatus.InitiateCase
                    If (GroupId = UserGroups.MedicalTechnician) Then
                        startPage = "PW Med Tech"
                    End If

                Case SpecCasePWWorkStatus.PackageReview
                    If (GroupId = UserGroups.AFRCHQTechnician) Then
                        startPage = "PW Documents"
                    End If

                Case SpecCasePWWorkStatus.MedicalReview
                    If (GroupId = UserGroups.BoardMedical) Then
                        startPage = "PW Board Med"
                    End If

                Case SpecCasePWWorkStatus.FinalReview
                    If (GroupId = UserGroups.AFRCHQTechnician) Then
                        startPage = "PW Board Med"
                    End If
        ...
        Public Sub InitPageAccess(ByVal specCase As SC_PWaivers)
        ...
        Public Function LoadCase(ByVal refId As Integer) As SC_PWaivers
            Dim scPW As SC_PWaivers = Nothing
            ...
            If (scPW Is Nothing) Then
                'we failed to load the case, nothing we can do
                ALOD.Logging.LogManager.LogError("NULL SpecCasePWaiver Error loading Case of Type (" + "SpecCasePWaiver" + ") with refId: " + refId.ToString())
                SetErrorMessage(Resources.Messages.ERROR_LOADING_CASE)
                Response.Redirect(Resources._Global.StartPage, True)
            End If
        ...
        Private Sub InitNavigator(ByVal refId As Integer)
            ...
            userAccess = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCasePW)
