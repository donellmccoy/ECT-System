...
Namespace Web.Special_Case.RS
    ...
    Partial Class Secure_sc_rs_NextAction
        ...
        Private boardStages As Short() = New Short() {
            CShort(SpecCaseRSStatusCode.InitiateCase),
            CShort(SpecCaseRSStatusCode.MedicalReview),
            CShort(SpecCaseRSStatusCode.FinalReview),
            CShort(SpecCaseRSStatusCode.InitiateCase)}
        ...
        Private unitStages As Short() = New Short() {
            CShort(SpecCaseRSStatusCode.RecruiterComments)}

#Region "SC_RSProperty"
...
#End Region
        ...
        Private Sub ShowRLB()
            Dim access As ALOD.Core.Domain.Workflow.PageAccessType = SectionList(RSSectionNames.RS_RLB.ToString())
...
#Region "ChangeStatusAndActions"
        ...
        Private Sub ProcessDispositions(newStatus As Integer)
            ...
            If (newStatus <> SpecCaseRSWorkStatus.InitiateCase AndAlso newStatus <> SpecCaseRSWorkStatus.FinalReview) Then
                Exit Sub
            End If
            ...
            ' Determine if the StampDocId needs to be cleared out to for a re-upload by the HQ Tech...
            If (Not SpecCase.PreviousDisposition.HasValue) Then
                If (SpecCase.med_off_approved.Value <> SpecCase.NormalizeHQTechDisposition(New NHibernateDaoFactory().GetLookupDispositionDao())) Then
                    SpecCase.StampedDocId = Nothing
                    SpecCase.PreviousDisposition = SpecCase.med_off_approved
                End If
            Else
                If (SpecCase.med_off_approved.Value <> SpecCase.PreviousDisposition.Value) Then
                    SpecCase.StampedDocId = Nothing
                    SpecCase.PreviousDisposition = SpecCase.med_off_approved
                End If
            End If
