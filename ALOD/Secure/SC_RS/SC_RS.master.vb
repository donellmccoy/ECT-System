...
Namespace Web.Special_Case.RS
    ...
    Partial Class SC_RSMaster
        Inherits System.Web.UI.MasterPage

        Private _dao As ISpecialCaseDAO
        Private _sc As SC_RS = Nothing
        ...
        Public Function ConstructSeniorMedicalReviewerTabVisibilityArgs() As SeniorMedicalReviewerTabVisibilityArgs
            ...
            workStatuses.Add(SpecCaseRSWorkStatus.SeniorMedicalReview)

            args.RefId = SpecCase.Id
            args.ModuleId = SpecCase.moduleId
            args.WorkStatusIds = workStatuses
            args.TabTitle = "RS Senior Med"
            args.Steps = Navigator.Steps

            Return args
        End Function

        Public Function GetStartPageTitle(ByVal workStatus As SpecCaseRSWorkStatus) As String
            Dim startPage As String = "RS Member"
            ...
            Select Case workStatus
                Case SpecCaseRSWorkStatus.InitiateCase
                    If (GroupId = UserGroups.AFRCHQTechnician) Then
                        startPage = "RS Tech"
                    End If
                Case SpecCaseRSWorkStatus.MedicalReview
                    If (GroupId = UserGroups.BoardMedical) Then
                        startPage = "RS Board Med"
                    End If
                Case Else
                    startPage = "RS Member"
            End Select
        ...
        Public Sub InitPageAccess(ByVal specCase As SC_RS)
        ...
        Public Function LoadCase(ByVal refId As Integer) As SC_RS
            Dim scRS As SC_RS = Nothing
        ...
        Private Sub InitNavigator(ByVal refId As Integer)
            ...
            Dim scRS As New SC_RS

            Session("refId") = refId

            userAccess = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCaseRS)
