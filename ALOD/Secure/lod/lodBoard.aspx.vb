...
Namespace Web.LOD
    ...
    Partial Class Secure_lod_lodBoard
        ...
        Protected Const optionalFindings As String = InvestigationDecision.APPROVE + "," + InvestigationDecision.DISAPPROVE + "," + InvestigationDecision.FORMAL_INVESTIGATION
        Protected Const optionalInformalFindings As String = InvestigationDecision.APPROVE + "," + InvestigationDecision.DISAPPROVE + "," + InvestigationDecision.FORMAL_INVESTIGATION
...
#Region "PageProperty"
        ...
        Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
            ...
            bdfrmlLegalFindings.DoDecisionAutoPostBack = True
            bdMedicalFindings_v2.DoDecisionAutoPostBack = True
            bdAppAuthFindings_v2.DoDecisionAutoPostBack = True
            bdFrmlAppAuthFindings_v2.DoDecisionAutoPostBack = True
            fcfrmlAppAuth_v2.DoDecisionAutoPostBack = True
            bdfrmlLegalFindings_v2.DoDecisionAutoPostBack = True
            bdfrmlMedicalFindings_v2.DoDecisionAutoPostBack = True
            bdfrmlPersonnelFindings_v2.DoDecisionAutoPostBack = True
            bdLegalFindings.DoDecisionAutoPostBack = True
            bdMedicalFindings.DoDecisionAutoPostBack = True
            bdPersonnelFindings.DoDecisionAutoPostBack = True
            bdLegalFindings_v2.DoDecisionAutoPostBack = True
            bdPersonnelFindings_v2.DoDecisionAutoPostBack = True
            fcAppAuth_v2.DoDecisionAutoPostBack = True
            bdFormalRAFindings_v2.DoDecisionAutoPostBack = True
            bdFindings.DoDecisionAutoPostBack = True
            bdAppAuthFindings.DoDecisionAutoPostBack = True
            fcAppAuth.DoDecisionAutoPostBack = True
            fcfrmlAppAuth.DoDecisionAutoPostBack = True
        End Sub
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Not IsPostBack Then
                ' Check for specific Wsid values
                Dim wsid As Integer = SESSION_WS_ID(RefId)
                If wsid = 208 Or wsid = 215 Then
                    ' Set the AFRCBoard_v2 panel to visible and editable
                    AFRCBoard_v2.Visible = True
                    bdFindings_v2.SetReadOnly = False
                Else
                    ' Hide the AFRCBoard_v2 panel for other Wsid values
                    AFRCBoard_v2.Visible = False
                End If
            End If
...
#Region "BoardProperty"
        ...
        Public Property FormalAppointingAuthorityEnabled() As Boolean
            Get
                If (lod.Workflow = 1) Then
                    Return fcfrmlAppAuth.SetReadOnly
                Else
                    Return fcfrmlAppAuth_v2.SetReadOnly
                End If
            End Get
            Set(ByVal value As Boolean)
                If (lod.Workflow = 1) Then
                    fcfrmlAppAuth.SetReadOnly = Not (value)
                Else
                    fcfrmlAppAuth_v2.SetReadOnly = Not (value)
                End If
            End Set
        End Property

        Public Property FormalAppointingAuthorityVisible() As Boolean
            Get
                If (lod.Workflow = 1) Then
                    Return FrmlAppointingAuthority.Visible
                Else
                    Return FrmlAppointingAuthority_v2.Visible
                End If
            End Get
            Set(ByVal value As Boolean)
                If (lod.Workflow = 1) Then
                    FrmlAppointingAuthority.Visible = value
                Else
                    FrmlAppointingAuthority_v2.Visible = value
                End If
            End Set
        End Property
...
#Region "Load"
        ...
        Public Sub LoadData_v2()
            ...
            ' Set the findings in the AFRCBoard_v2 section
            If Not (cFinding Is Nothing) Then
            ...
            If (UserCanEdit AndAlso SESSION_GROUP_ID = 7) Then          '7 is Board Tech
                fcAppAuth_v2.SetReadOnly = False
            End If

            hasFinding = False

            Dim f_wsidList As New List(Of Integer)({215, 216, 217, 218, 294, 219})
            Dim wsidList As New List(Of Integer)({208, 209, 210, 211, 291, 212})
            'appointing authority
            If (f_wsidList.Contains(SESSION_WS_ID(RefId))) Then
                cFinding = lod_v2.FindByType(PersonnelTypes.FORMAL_APP_AUTH)
            Else
                cFinding = lod_v2.FindByType(PersonnelTypes.APPOINT_AUTH)
            End If

            LoadPreviousFindings_v3()
            cFinding = lod_v2.FindByType(PersonnelTypes.FORMAL_APP_AUTH)
            ...
            If Not (cFinding Is Nothing) Then
                If cFinding.DecisionYN <> String.Empty Then
                    fcAppAuth_v2.Decision = cFinding.DecisionYN
                    hasFinding = True
                End If
                If (Not cFinding.Finding Is Nothing AndAlso cFinding.Finding > 0) Then
            ...
            If (UserCanEdit And SESSION_GROUP_ID = 8) Then
                bdfrmlLegalFindings_v2.SetReadOnly = "False"
            End If
            If (UserCanEdit And SESSION_GROUP_ID = 8) Then
                bdLegalFindings_v2.SetReadOnly = "False"
            End If
            If Not (cFinding Is Nothing) Then
            ...
            If (UserCanEdit And SESSION_GROUP_ID = 9) Then
                bdMedicalFindings_v2.SetReadOnly = "False"
            End If
            ...
            If (UserCanEdit And SESSION_GROUP_ID = 97) Then
                bdPersonnelFindings_v2.SetReadOnly = "False"
            End If
            ...
            If (UserCanEdit And SESSION_GROUP_ID = 11) Then
                bdAppAuthFindings_v2.SetReadOnly = "False"
            End If
            ...
            ddlFormalApprovingAuthorities_v2.DataSource = UserService.GetUsersAlternateTitleByGroupCompo(ALOD.Core.Domain.Users.UserGroups.BoardApprovalAuthority, Session(SESSIONKEY_COMPO))
        ...
        Public Sub LoadPreviousFindings_v3()
            fcAppAuth_v2.PrevFindingsLableText = "IO Findings:"
            fcAppAuth_v2.PrevFindingsText = "Not found"
            Dim ioFinding As LineOfDutyFindings
            ioFinding = lod_v2.FindByType(PersonnelTypes.IO)
            If ioFinding IsNot Nothing Then
                If (ioFinding.Finding IsNot Nothing AndAlso ioFinding.Finding > 0) Then
                    fcAppAuth_v2.PrevFindingsText = ioFinding.Description
                End If
            End If
        End Sub
...
#Region "Save"
        ...
        Public Sub SaveData_v2()
            ...
            If (SESSION_GROUP_ID = 7) Then
                access = SectionList(SectionNames.BOARD_REV.ToString())
                cFinding.PType = PersonnelTypes.BOARD 'Board Tech
                cFinding.DecisionYN = fcAppAuth_v2.Decision
                If fcAppAuth_v2.Decision <> "Y" AndAlso fcAppAuth_v2.Findings IsNot Nothing AndAlso fcAppAuth_v2.Findings > 0 Then
                    cFinding.Finding = fcAppAuth_v2.Findings
                End If
                cFinding.Explanation = fcAppAuth_v2.Remarks
                cFinding.FindingsText = fcAppAuth_v2.FormFindingsText
                lod_v2.SetFindingByType(cFinding)
            End If

            If (SESSION_GROUP_ID = 8) Then
                access = SectionList(SectionNames.BOARD_LEGAL_REV.ToString())
                'If access = ALOD.Core.Domain.Workflow.PageAccessType.ReadWrite Then
                'If () Then
                cFinding.PType = PersonnelTypes.BOARD_JA
            ...
            '    If (fcAppAuth_v2.FindingsIndex >= 0) Then
            '    lod_v2.
            'End If
            ...
            'final Board Review
            If (SESSION_GROUP_ID = 7 AndAlso lod.Formal = True) Then
                access = SectionList(SectionNames.FORMAL_BOARD_REV.ToString())
                If access = ALOD.Core.Domain.Workflow.PageAccessType.ReadWrite Then
        ...
        Public Sub SaveData_V3()
            ...
            cFinding = CreateFinding(lod_v2.Id)
            'Board Legal
            If (UserCanEdit AndAlso SESSION_GROUP_ID = 8) Then

                If (SESSION_WS_ID(RefId) = 217) Then            'formal
                    cFinding.PType = PersonnelTypes.FORMAL_BOARD_JA
                    cFinding.Finding = bdLegalFindings_v2.Findings
                    cFinding.DecisionYN = bdLegalFindings_v2.Decision
                    cFinding.FindingsText = bdLegalFindings_v2.FormFindingsText
                ...
                ElseIf (SESSION_WS_ID(RefId) = 209) Then        'informal
                    cFinding.PType = PersonnelTypes.BOARD_JA
                    cFinding.Finding = bdLegalFindings_v2.Findings
                    cFinding.DecisionYN = bdLegalFindings_v2.Decision
                    cFinding.FindingsText = bdLegalFindings_v2.FormFindingsText
            ...
            End If
            'Appointing Authority
            If (UserCanEdit AndAlso SESSION_GROUP_ID = 7) Then
                If (SESSION_WS_ID(RefId) = 215) Then            'formal
                    cFinding.PType = PersonnelTypes.FORMAL_APP_AUTH
                    cFinding.Finding = fcAppAuth_v2.Findings
                    cFinding.FindingsText = fcAppAuth_v2.FormFindingsText
                    cFinding.Explanation = fcAppAuth_v2.Remarks
                    cFinding.DecisionYN = fcAppAuth_v2.Decision

                ElseIf (SESSION_WS_ID(RefId) = 208) Then            'informal
                    cFinding.PType = PersonnelTypes.APPOINT_AUTH
                    cFinding.Finding = fcAppAuth_v2.Findings
                    cFinding.FindingsText = fcAppAuth_v2.FormFindingsText
                    cFinding.Explanation = fcAppAuth_v2.Remarks
                    cFinding.DecisionYN = fcAppAuth_v2.Decision
                    'cFinding.bdLegalFindings_v2
                End If
            End If
            'Board Medical
            If (UserCanEdit AndAlso SESSION_GROUP_ID = 9) Then
                If (SESSION_WS_ID(RefId) = 216) Then        'formal
                    cFinding.PType = PersonnelTypes.FORMAL_BOARD_SG
                ElseIf (SESSION_WS_ID(RefId) = 210) Then        'informal
                    cFinding.PType = PersonnelTypes.BOARD_SG
                End If
                cFinding.Finding = bdMedicalFindings_v2.Findings
                cFinding.DecisionYN = bdMedicalFindings_v2.Decision
                ...
                cFinding.Explanation = bdMedicalFindings_v2.Remarks
            End If
            'Board Personnel / Board Admin
            If (UserCanEdit AndAlso SESSION_GROUP_ID = 97) Then
                If (SESSION_WS_ID(RefId) = 218) Then        'formal
                    cFinding.PType = PersonnelTypes.FORMAL_BOARD_A1
                ElseIf (SESSION_WS_ID(RefId) = 211) Then        'informal
                    cFinding.PType = PersonnelTypes.BOARD_A1
                End If
                cFinding.DecisionYN = bdPersonnelFindings_v2.Decision
                If (bdPersonnelFindings_v2.Decision.Equals("Y")) Then
                    cFinding.Finding = bdMedicalFindings_v2.Findings
                Else
                    cFinding.Finding = bdPersonnelFindings_v2.Findings
                End If
            ...
            End If
            'Approving Authority
            If (UserCanEdit AndAlso SESSION_GROUP_ID = 11) Then
                 If (SESSION_WS_ID(RefId) = 219) Then        'formal
                    cFinding.PType = PersonnelTypes.FORMAL_BOARD_AA
                    cFinding.Finding = bdFrmlAppAuthFindings_v2.Findings
                    cFinding.DecisionYN = bdFrmlAppAuthFindings_v2.Decision
                    cFinding.FindingsText = bdFrmlAppAuthFindings_v2.FormFindingsText
                    cFinding.Explanation = bdFrmlAppAuthFindings_v2.Remarks
                ElseIf (SESSION_WS_ID(RefId) = 212) Then        'informal
                    cFinding.PType = PersonnelTypes.BOARD_AA
                    cFinding.Finding = bdAppAuthFindings_v2.Findings
                    cFinding.DecisionYN = bdAppAuthFindings_v2.Decision
                    cFinding.FindingsText = bdAppAuthFindings_v2.FormFindingsText
                    cFinding.Explanation = bdAppAuthFindings_v2.Remarks
                End If
            End If
            'cFinding.Explanation = bdAppAuthFindings_v2.Remarks
