...
#Region "Page Methods"
        ...
        Private Sub DocumentationQuestionLogic()
            ...
            ' Logic for rblEightYearRule  -- 09/14/2023 Diamante Lawson ,Users wanted some questions removed--
            If (rblEightYearRule.SelectedIndex >= 0) Then
                ...
                ''    rblCorrectlyIdentified.Enabled = True
                ''End If

                ''If (rblCorrectlyIdentified.SelectedIndex >= 0) Then
                ''    rblVerifiedAndAttached.Enabled = True
                ''End If
                ''If (rblVerifiedAndAttached.SelectedIndex >= 0) Then
                ''    rblIDTStatus.Enabled = True
                ''End If

                ''If (rblIDTStatus.SelectedIndex >= 0) Then
                ''rblIPCARSAttached.Enabled = True
                ''End If
                ''If (rblIPCARSAttached.SelectedIndex >= 0) Then
                '' Uncomment the following line to enable "8 Year Rule" related logic
                ...
                ''    lblPCARSAttached.Text = "PCARS Report must be attached to the MilPDS in order for the case to continue"
                ''End If
                ...
                '' Uncomment the following line to enable "Status Worsened" related logic
        ...
        Private Sub InitControls()
            ...
            If (SESSION_GROUP_ID = 96) Then
                ComponentPanel.Visible = False
                lblAppointedIOIATrainingRow_v2.Text = "E"
                lblAppointedIOEmailRow_v2.Text = "F"
            End If
...
#Region "LOD"
        ...
        Public Sub LoadAppointingAuthority()
            ...
            ucInformalAndReinvestigationFindings.LoadFindingOptionsByWorkflow(LOD.Workflow, UserGroups.WingCommander Or LOD.CurrentStatusCode = LodStatusCode.AppointingAutority_LODV3)
        ...
        Protected Sub FindingSelected(sender As Object, e As RadioButtonSelectedEventArgs)
            ...
            If (LOD.CurrentStatusCode = LodStatusCode.AppointingAutority_LODV3 Or LOD.CurrentStatusCode = LodStatusCode.AppointingAutorityReview AndAlso
        ...
        Private Sub DisplayReadWrite()
            ...
            If (LOD.CurrentStatusCode = LodStatusCode.AppointingAutorityReview Or LOD.CurrentStatusCode = LodStatusCode.AppointingAutority_LODV3) Then
...
#Region "LOD_v2"
        ...
        Public Sub LoadAppointingAuthority_v2()
            ...
            ucInformalAndReinvestigationFindings_v2.LoadFindingOptionsByWorkflow(LOD.Workflow, UserGroups.WingCommander Or LOD.CurrentStatusCode = LodStatusCode.AppointingAutority_LODV3, optionalInformalFindings, False)
        ...
        Public Sub LoadFormalAppointingAuthority_v2(ByVal isReadOnly As Boolean)
            ...
            FormalFindings_v2.RemoveFinding("Recommend Formal LOD Investigation")
        ...
        Public Sub LoadRLodFormalAppointingAuthorityReadOnly_v2()
            ...
            If (finding IsNot Nothing) Then
                ...
                If (scope = AccessScope.Compo) OrElse (group = UserGroups.WingCommander Or LOD.CurrentStatusCode = LodStatusCode.AppointingAutority_LODV3) OrElse UserHasPermission("exePostCompletion") Then
                    IoPanel_v2.Visible = True
        ...
        Public Sub LoadWingCC()
            Dim finding As LineOfDutyFindings
            finding = LOD.FindByType(PersonnelTypes.APPOINT_AUTH)

            If (finding IsNot Nothing) Then
                ' Logic for rblEightYearRule  -- Diamante Lawson 09/14/2023--
                If (finding.EightYearRule.HasValue) Then
                    rblEightYearRule.SelectedValue = finding.EightYearRule
                End If

                ' Logic for rblPriorToDutytatus
                If (finding.PriorToDutytatus.HasValue) Then
                    rblPriorToDutytatus.SelectedValue = finding.PriorToDutytatus
                End If
            End If

            'If (finding IsNot Nothing) Then
            '    'If (finding.LODInitiation.HasValue) Then
            '    '    rblLODInitiation.SelectedValue = finding.LODInitiation
            'End If
            ''If (finding.WrittenDiagnosis.HasValue) Then
            ''    rblWrittenDiagnosis.SelectedValue = finding.WrittenDiagnosis
            ''End If
            ''If (finding.MemberRequest.HasValue) Then
            ''    rblMemberRequest.SelectedValue = finding.MemberRequest
            ''End If
            ''If (finding.CorrectlyIdentified IsNot Nothing AndAlso Not finding.CorrectlyIdentified.Equals("")) Then
            ''        rblCorrectlyIdentified.SelectedValue = finding.CorrectlyIdentified
            ''    End If
            ''If (finding.VerifiedAndAttached IsNot Nothing AndAlso Not finding.VerifiedAndAttached.Equals("")) Then
            ''        rblVerifiedAndAttached.SelectedValue = finding.VerifiedAndAttached
            ''    End If
            ''    If (finding.IDTStatus IsNot Nothing AndAlso Not finding.IDTStatus.Equals("")) Then
            ''        rblIDTStatus.SelectedValue = finding.IDTStatus
            ''    End If
            ''If (finding.IPCARSAttached.HasValue) Then
            ''    rblIPCARSAttached.SelectedValue = finding.IPCARSAttached
            ''End If
            'If (finding.EightYearRule.HasValue) Then
            '    rblEightYearRule.SelectedValue = finding.EightYearRule
            'End If
            ...
            '    rblPriorToDutytatus.SelectedValue = finding.PriorToDutytatus
            'End If
            ''If (finding.StatusWorsened IsNot Nothing AndAlso Not finding.StatusWorsened.Equals("")) Then
            ''    rblStatusWorsened.SelectedValue = finding.StatusWorsened
            ''End If
            ''End If
        End Sub
        ...
        Protected Sub FindingSelected_v2(sender As Object, e As RadioButtonSelectedEventArgs)
            ...
            lblIOLabel_v2.CssClass = String.Empty
            lblIORow_v2.Text = String.Empty
            lblIOCompletedByRow_v2.Text = String.Empty
            lblIOInstructionsRow_v2.Text = String.Empty
            lblIOPOCRow_v2.Text = String.Empty

            If (ucInformalAndReinvestigationFindings_v2.Findings = LodFinding.NotInLineOfDutyNotDueToOwnMisconduct) Then
            ...
            ElseIf ((ucInformalAndReinvestigationFindings_v2.Findings = LodFinding.RecommendFormalInvestigation) Or
                LookupService.GetIsReinvestigationLod(LOD.Id)) Then
        ...
        'Zach 1/24/19 added ByRef
        Private Sub AssignSubmittedDataToInvestigatingOfficer_v2(ByRef ioUser As AppUser, hasAlreadyBeenModified As Boolean)
            Dim wasModified As Boolean = hasAlreadyBeenModified
        ...
        Private Sub DisplayReadOnly_v2()
            ...
            If (ws_Id > 324) Then
                lblAAEditPanelHeader_v2.Text = "1 - Appointing Authority Review"
                pnlDocumentationQuestions.Visible = False
            End If
            If (SESSION_WS_ID >= 329) Then
                LoadWingCC()
            End If
            If (isRLod = False) Then
        ...
        Private Sub SaveWingCC(ByVal findings As LineOfDutyFindings)

            ' Logic for rblEightYearRule
            If (rblEightYearRule.SelectedIndex >= 0) Then
                findings.EightYearRule = Convert.ToBoolean(rblEightYearRule.SelectedValue.ToString())
            End If

            ' Logic for rblPriorToDutytatus
            If (rblPriorToDutytatus.SelectedIndex >= 0) Then
                findings.PriorToDutytatus = Convert.ToBoolean(rblPriorToDutytatus.SelectedValue.ToString())
            End If

            ''If (rblLODInitiation.SelectedIndex >= 0) Then
            ''    findings.LODInitiation = Convert.ToBoolean(rblLODInitiation.SelectedValue.ToString())
            ''End If
            ''If (rblWrittenDiagnosis.SelectedIndex >= 0) Then
            ''    findings.WrittenDiagnosis = Convert.ToBoolean(rblWrittenDiagnosis.SelectedValue.ToString())
            ''End If
            ''If (rblMemberRequest.SelectedIndex >= 0) Then
            ''    findings.MemberRequest = Convert.ToBoolean(rblMemberRequest.SelectedValue.ToString())
            ''End If
            ''If (rblCorrectlyIdentified.SelectedIndex >= 0) Then
            ''    findings.CorrectlyIdentified = rblCorrectlyIdentified.SelectedValue.ToString()
            ''End If
            ''If (rblVerifiedAndAttached.SelectedIndex >= 0) Then
            ''    findings.VerifiedAndAttached = rblVerifiedAndAttached.SelectedValue.ToString()
            ''End If
            ''If (rblIDTStatus.SelectedIndex >= 0) Then
            ''    findings.IDTStatus = rblIDTStatus.SelectedValue.ToString()
            ''End If
            ''If (rblIPCARSAttached.SelectedIndex >= 0) Then
            ''    findings.IPCARSAttached = Convert.ToBoolean(rblIPCARSAttached.SelectedValue.ToString())
            ''End If
            'If (rblEightYearRule.SelectedIndex >= 0) Then
            '    findings.EightYearRule = Convert.ToBoolean(rblEightYearRule.SelectedValue.ToString())
            'End If
            ...
            '    findings.PriorToDutytatus = Convert.ToBoolean(rblPriorToDutytatus.SelectedValue.ToString())
            'End If
            'If (rblStatusWorsened.SelectedIndex >= 0) Then
            ''findings.StatusWorsened = rblStatusWorsened.SelectedValue.ToString()
            ''End If
        End Sub
...
#Region "Shared Methods"
        ...
        Private Sub UpdateLodInvestigationAssignedIo(ioUser As AppUser)
            If (ioUser Is Nothing OrElse LOD Is Nothing OrElse LOD.LODInvestigation Is Nothing) Then
                Exit Sub
            End If
