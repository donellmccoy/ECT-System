...
#End Region
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If (Not Page.IsPostBack) Then
                ...
                UserCanEdit = GetAccessLOD(Navigator.PageAccess, False, lod)
        ...
        Private Sub DocumentationQuestionLogic()
            ...
            ' Enable rblPriorToDutytatus --- 09/14/2023 Diamante Lawson -- this question seeems to be attached to the document page--
            rblPriorToDutytatus.Enabled = True

            ' If no value is selected in rblPriorToDutytatus, disable MedOfficer_v2
            If rblPriorToDutytatus.SelectedIndex < 0 Then
                MedOfficer_v2.Enabled = True
            End If
...
#Region "LOD_v2"
        ...
        Protected Sub DisplayReadOnly_v2(ByVal medical As LineOfDutyMedical_v2)
            ...
            If (Not IsDBNull(LineOfDutyUnitDao.GetById(refId, False).DutyFrom)) Then
                If (Not IsDBNull(LineOfDutyUnitDao.GetById(refId, False).DutyTo)) Then
                    If (LineOfDutyUnitDao.GetById(refId, False).DutyFrom IsNot Nothing) Then
                        If (LineOfDutyUnitDao.GetById(refId, False).DutyTo IsNot Nothing) Then
                            Dim unit As LineOfDutyUnit_v2 = LineOfDutyUnitDao.GetById(refId, False)
                            If (unit.DutyFrom.HasValue) Then
                                ComponentFromDate_v2.Text = unit.DutyFrom.Value.ToString(DATE_FORMAT)
                                ComponentFromTime_v2.Text = unit.DutyFrom.Value.ToString(HOUR_FORMAT)
                            End If
                            If (unit.DutyTo.HasValue) Then
                                ComponentToDate_v2.Text = unit.DutyTo.Value.ToString(DATE_FORMAT)
                                ComponentToTime_v2.Text = unit.DutyTo.Value.ToString(HOUR_FORMAT)
                            End If
                        End If
                    End If
                End If
            ...
            If (ws_Id > 324) Then
                ...
                rblPriorToDutytatus.Visible = True
                'rblStatusWorsened.Visible = False
                '    Try
                '        'SetRadioLables(medical.LODInitiation, lblLODInitiation)
                '        SetRadioLables(medical.WrittenDiagnosis, lblWrittenDiagnosis)
                '        SetRadioLables(medical.MemberRequest, lblMemberRequest)
                SetRadioLables(medical.PriorToDutytatus, lblPriorToDutytatus)
                'SetRadioLables(medical.StatusWorsened, lblStatusWorsened)
                '    Catch
        ...
        Protected Sub DisplayReadWrite_v2(ByVal medical As LineOfDutyMedical_v2)
            ...
            If (String.IsNullOrEmpty(medical.NatureOfIncidentDescription)) Then
                DeathCaseChoice_v2.Enabled = True
            Else
                If (medical.NatureOfIncidentDescription.Equals("Death")) Then
                    DeathCaseChoice_v2.SelectedValue = "Yes"
                Else
                    DeathCaseChoice_v2.SelectedValue = "No"
                End If
                DeathCaseChoice_v2.Enabled = False
            End If

            SetRadioList(MvaInvolvedChoice_v2, medical.MvaInvolved)
            If (IncidentTypeSelect_v2.SelectedItem.Text.Equals("Injury MVA")) Then
                MvaInvolvedChoice_v2.SelectedValue = "Yes"
            Else
                MvaInvolvedChoice_v2.SelectedValue = "No"
            End If
            MvaInvolvedChoice_v2.Enabled = False

            BoardFinalizationrbl_v2.Enabled = False

            SetBoardFinalizationToNo()
            ...
            If (ws_Id > 324) Then
                ...
                '    SetRadioList(rblLODInitiation, medical.LODInitiation)
                '    SetRadioList(rblWrittenDiagnosis, medical.WrittenDiagnosis)
                '    SetRadioList(rblMemberRequest, medical.MemberRequest)         ---Task 1 Changes Diamante Lawson ---
                SetRadioList(rblPriorToDutytatus, medical.PriorToDutytatus)
                'SetRadioList(rblStatusWorsened, medical.StatusWorsened)
            End If
            ...
            If ((CInt(Session("groupId")) = UserGroups.MedicalOfficer Or CInt(Session("groupId") = 119))) Then
        ...
        Protected Sub IncidentTypeSelect_v2_SelectedIndexChanged(sender As Object, e As EventArgs) Handles IncidentTypeSelect_v2.SelectedIndexChanged
            If (IncidentTypeSelect_v2.SelectedItem.Text.Equals("Death")) Then
                DeathCaseChoice_v2.SelectedValue = "Yes"
            Else
                DeathCaseChoice_v2.SelectedValue = "No"
            End If
        ...
        Private Sub HideEditableControls_v2()
            ...
            ComponentFromDate_v2.Enabled = False
            ComponentFromTime_v2.Enabled = False
            ComponentToDate_v2.Enabled = False
            ComponentToTime_v2.Enabled = False
        ...
        Private Sub InitCategorySelectDropdownList_v2()
            ...
            If Session(SESSIONKEY_COMPO) = "6" Then
                CategorySelect_v2.DataSource = From n In LookupDao.GetCategory() Select n Where n.Value < 5
            Else
                CategorySelect_v2.DataSource = From n In LookupDao.GetCategory() Select n Where n.Value >= 5
            End If
        ...
        Private Sub InitComponentSelectDropdownList_v2()
            ...
            If Session(SESSIONKEY_COMPO) = "6" Then
                ...
                End If
            Else 'zach
                ComponentSelect_v2.DataSource = From n In LookupDao.GetComponents() Select n Where n.Value = 3
                ComponentSelect_v2.DataTextField = "Name"
                ComponentSelect_v2.DataValueField = "Value"
                ComponentSelect_v2.DataBind()
            End If
        ...
        Private Sub SaveData_v2()
            ...
            '    medicalInfo.LODInitiation = Convert.ToBoolean(rblLODInitiation.SelectedValue)
            'End If
            'If (Not rblWrittenDiagnosis.SelectedValue.Equals("")) Then
            '    medicalInfo.WrittenDiagnosis = Convert.ToBoolean(rblWrittenDiagnosis.SelectedValue)
            'End If
            'If (Not rblMemberRequest.SelectedValue.Equals("")) Then
            '    medicalInfo.MemberRequest = Convert.ToBoolean(rblMemberRequest.SelectedValue)
            'End If
            If (Not rblPriorToDutytatus.SelectedValue.Equals("")) Then
                medicalInfo.PriorToDutytatus = Convert.ToBoolean(rblPriorToDutytatus.SelectedValue)
            End If
            'If (Not rblStatusWorsened.SelectedValue.Equals("")) Then
            '    medicalInfo.StatusWorsened = rblStatusWorsened.SelectedValue
            'End If
        ...
        Private Sub SetBoardFinalization()
            If (MemberConditionSelect_v2.SelectedValue.Equals("was not") AndAlso DeathCaseChoice_v2.SelectedValue = "No" AndAlso MvaInvolvedChoice_v2.SelectedValue = "No") Then
            ...
            If (MemberConditionSelect_v2.SelectedValue.Equals("was under") AndAlso DeathCaseChoice_v2.SelectedValue = "Yes" AndAlso MvaInvolvedChoice_v2.SelectedValue = "Yes") Then
                SetRadioList(BoardFinalizationrbl_v2, "Yes")
        ...
        Private Sub SetBoardFinalizationToNo()
            Dim numbers = New List(Of Integer)({118, 119, 120, 121, 122})
            If (numbers.Contains(CInt(Session("groupId")))) Then
                SetRadioList(BoardFinalizationrbl_v2, "No")
                BoardFinalizationrbl_v2.Enabled = False
            End If
        End Sub
