...
#Region "PageProperty"
        ...
        Protected Sub Page_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
            ...
            If (LOD.Workflow = 27 AndAlso LOD.WorkflowStatus.StatusCodeType.Id = LodStatusCode.MedTechReview) Then                    '(LOD.WorkflowStatus.StatusCodeType.Id = LodStatusCode.MedTechReview Or LOD.WorkflowStatus.StatusCodeType.Id = LodStatusCode.MedicalOfficer_LODV3
            ...
            'Create a method that adds audit validation errors for the "error"
            GetAuditValidations()
...
#End Region
        ...
        Protected Sub CheckRoutingSelection()
            ...
            ElseIf (workOption.DisplayText Like "Direct Formal Investigation*") Then
                ...
                commentsRow.Visible = True
                commentLetter.Text = "B"
            ElseIf (workOption.DisplayText Like "Request ARC SME Consult") Then
                ...
                commentsRow.Visible = True
                commentLetter.Text = "B"
            ElseIf (workOption.DisplayText Like "Request Wing JA Consult") Then
        ...
        Protected Function GetStatusInfo(ByVal statusCode As Integer) As Tuple(Of Integer, String, Integer, Integer)
            ...
            Select Case statusCode
                ...
                Case LodStatusCode.AppointingAutority_LODV3
                    sentTo = "Appointing Autority (P)"
                    sentToId = UserGroups.AppointingAuthority_Pilot
                    ptype = PersonnelTypes.APPOINT_AUTH
                    access = SectionList(SectionNames.IO.ToString())
                Case Else
                    sentTo = ""
        ...
        Protected Sub rbtOption_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles rbtOption.ItemDataBound
            ...
            If (e.Item.ItemType = ListItemType.Item) OrElse (e.Item.ItemType = ListItemType.AlternatingItem) Then
                ...
                Dim modules As System.Web.UI.WebControls.Repeater
                modules = e.Item.Parent
                Dim list As List(Of String) = New List(Of String)
                Dim index As Integer

                ' Check if modules.DataSource is not null
                If modules.DataSource IsNot Nothing Then
                    For Each x As WorkflowStatusOption In modules.DataSource
                        list.Add(x.DisplayText)
                    Next
                End If

                options = e.Item.DataItem
                index = list.FindIndex(Function(x As String) x.Contains("Legacy"))
                rb.Text = Server.HtmlEncode(options.DisplayText)

                ' Check if LOD is not null
                If LOD IsNot Nothing Then
                    Dim dateCreated As String = LOD.CaseId.Substring(0, 4).ToString() + "-" + LOD.CaseId.Substring(4, 2).ToString() + "-" + LOD.CaseId.Substring(6, 2).ToString()
                    ' Rest of the code that uses LOD
                End If

                If (index > 0 And options.DisplayText Like "Complete") Then
                    rb.Visible = False
                Else
                ...
                If (index < 0 And options.DisplayText Like "Complete") Then
                    ' Check if LOD is not null before using it
                    If LOD IsNot Nothing Then
                        Dim dateCreated As String = LOD.CaseId.Substring(0, 4).ToString() + "-" + LOD.CaseId.Substring(4, 2).ToString() + "-" + LOD.CaseId.Substring(6, 2).ToString()
                        If (dateCreated > Convert.ToDateTime("2022-03-13")) Then
                            If (options.wsStatusOut <> 330) Then
                                rb.Visible = False
                            End If
                        ElseIf (dateCreated < Convert.ToDateTime("2022-03-13")) Then
                            If (options.wsStatusOut <> 220) Then
                                rb.Visible = False
                            End If
                        End If
                    End If
                ...
                ' Check if finding is not null
                If finding IsNot Nothing Then
                    Try ' CodeCleanup
                        If (index < 0 And options.DisplayText Like "Forward to LOD Board") Then
                            If finding.Description Like "Not ILOD - Not Due To Own Misconduct" And
                                finding.Description Like "By Reason of EPTS-NSA" Then
                                rb.Visible = True
                            End If
                        End If
                    Catch
                        ' Handle any exceptions that may occur in the try-catch block
                    End Try
                End If
                ...
                If finding IsNot Nothing Then
                    Try ' CodeCleanup
                        If (index < 0 And options.DisplayText Like "Forward to LOD Board") Then
                            If (finding.Description Like "Not ILOD - Not Due To Own Misconduct" And
                                finding.Description Like "By Reason of Absent without Authority") OrElse
                                finding.Description Like "Recommend Formal Investigation" Then
                                rb.Visible = False
                            End If
                        End If
                    Catch
        ...
        Protected Sub SignButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles SignButton.Click
            ...
            If (workOption.DisplayText Like "RWOA*") OrElse (workOption.DisplayText Like "RFA*") Then
                ...
                If (workOption.DisplayText Like "RWOA for reply*") OrElse (workOption.DisplayText Like "RFA for reply*") Then
                    LOD_v2.DirectReply = True
                End If

                If (workOption.DisplayText Like "RWOA for rerouting*") OrElse (workOption.DisplayText Like "RFA for rerouting*") Then
                    LOD_v2.DirectReply = False
                End If
        ...
        Protected Sub SignStarted(ByVal sender As Object, ByVal e As SignStartedEventArgs) Handles SigBlock.SignStarted
            ...
            'If (LOD.CurrentStatusCode = LodStatusCode.AppointingAutority_LODV3) Then
            '    If (statusCodeOut = LodStatusCode.NotifyFormalInvestigator) Then
            '        If (Not SaveIO()) Then
            '            LogManager.LogError("An error occured saving IO.  Id: " + LOD.Id.ToString())
            '            e.Cancel = True
            '        End If
        ...
        Private Sub InitNextActionOptions()
                        ...
                        If (r.RuleTypes.ruleTypeId = RuleKind.Visibility) Then
                            If (r.RuleTypes.Name.ToLower().Equals("checksignature")) Then
                                ...
                                ElseIf (data.Count > 1) Then
                                    If (data(1).Trim().Equals("True")) Then
                                        o.OptionVisible = False
                                    End If
                                End If
            ...
            '    'Formal AFRC LOD Board Review (ANG Board Technician)
            '    If LOD.CurrentStatusCode = LodStatusCode.FormalBoardReview Then 'formal lod, 215

            '        For Each o In options
            '            'Remove Complete and all RWOA Options
            '            If o.Id = 607 OrElse o.Id = 608 OrElse o.Id = 609 OrElse o.Id = 610 OrElse o.Id = 611 OrElse o.Id = 612 OrElse o.Id = 693 Then
            '                o.OptionValid = False
            '                o.OptionVisible = False
            '            End If
            ...
            '    ElseIf LOD.CurrentStatusCode = LodStatusCode.BoardReview Then 'normal lod, 208

            '        For Each o In options
            '            'Remove Complete and all RWOA Options
            '            If o.Id = 577 OrElse o.Id = 578 OrElse o.Id = 579 OrElse o.Id = 580 OrElse o.Id = 581 OrElse o.Id = 582 OrElse o.Id = 583 _
            '                OrElse o.Id = 692 OrElse o.Id = 694 OrElse o.Id = 695 OrElse o.Id = 696 OrElse o.Id = 697 OrElse o.Id = 698 OrElse o.Id = 799 Then
            '                o.OptionValid = False
            '                o.OptionVisible = False
            '            End If
            ...
            '    If LOD.CurrentStatusCode = LodStatusCode.FormalBoardPersonnelReview Then 'formnal lod, 218
            ...
            '    ElseIf LOD.CurrentStatusCode = LodStatusCode.BoardPersonnelReview Then 'formnal lod, 211
        ...
        '            rb.Visible = False
        '        Else
        '            rb.Visible = options.OptionVisible                      -- This Sub Could not handle nulls to the system made changes above to allow on this page Diamante Lawson 09/19/2023 --
        '        End If
        ...
        '        Dim finding As LineOfDutyFindings = LOD.FindByType(pType)
        '        Try 'CodeCleanup
        '            If (index < 0 And options.DisplayText Like "Forward to LOD Board" And finding.Description Like "Not ILOD - Not Due To Own Misconduct") Then
        '                rb.Visible = False
        '            End If
        '        Catch
...
#Region "AddPersonnelInformation"
        ...
        Public Sub InsertPersonnelInfo(ByVal StatusCodeIn As Integer)
            ...
            If (GetStatusInfo(StatusCodeIn).Item3 > 0 AndAlso GetStatusInfo(StatusCodeIn).Item3 = PersonnelTypes.APPOINT_AUTH AndAlso SESSION_WS_ID(refId) = 329) Then
                'cFinding.Name = UserService.CurrentUser.FirstLastName
                'cFinding.Pascode = UserService.CurrentUser.Unit.PasCode
                LODDao.SaveAAId(SESSION_USER_ID, refId)
            End If
...
#Region "AuditValidations"

        Protected Sub GetAuditValidations()
            Dim auditInfo As LineOfDutyAuditDao = New LineOfDutyAuditDao
            Dim data As New DataSet
            data = auditInfo.GetAuditInfo(refId)
            Dim auditData As DataTable = data.Tables.Item(0)
            'LOD Audit (A1) Approving Authority/SAF MRR Questions
            If (SESSION_WS_ID(refId) = 335) Then
                If (IsDBNull(auditData.Rows.Item(0).Item("StatusValidated"))) Then
                    errors.Add(New ValidationItem("Audit", "Approving Authority/SAF MRR Questions", "Validate Status"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("LODInitiation"))) Then
                    errors.Add(New ValidationItem("Audit", "Approving Authority/SAF MRR Questions", "LOD Initiation Form"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("WrittenDiagnosis"))) Then
                    errors.Add(New ValidationItem("Audit", "Approving Authority/SAF MRR Questions", "Written Diagnosis"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("MemberRequest"))) Then
                    errors.Add(New ValidationItem("Audit", "Approving Authority/SAF MRR Questions", "Member Request"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("IncurredOrAggravated"))) Then
                    errors.Add(New ValidationItem("Audit", "Approving Authority/SAF MRR Questions", "Injury incurred or aggravated"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("IllnessOrDisease"))) Then
                    errors.Add(New ValidationItem("Audit", "Approving Authority/SAF MRR Questions", "Illness or Disease"))
                    SignButton.Enabled = False
                End If
                If (Not IsDBNull(auditData.Rows.Item(0).Item("Determination"))) Then
                    If (auditData.Rows.Item(0).Item("Determination") = 0 And IsDBNull(auditData.Rows.Item(0).Item("A1_DeterminationNotCorrect"))) Then
                        errors.Add(New ValidationItem("Audit", "Approving Authority/SAF MRR Questions", "Reason Informal LOD determination was not correct"))
                        SignButton.Enabled = False
                    End If
                ElseIf (IsDBNull(auditData.Rows.Item(0).Item("Determination"))) Then
                    errors.Add(New ValidationItem("Audit", "Approving Authority/SAF MRR Questions", "Was Informal LOD determination correct"))
                    SignButton.Enabled = False
                End If
                If (Not IsDBNull(auditData.Rows.Item(0).Item("StatusValidated"))) Then
                    If (auditData.Rows.Item(0).Item("StatusValidated") = 0 And auditData.Rows.Item(0).Item("StatusOfMember") = 0 And auditData.Rows.Item(0).Item("Orders") = 0 And auditData.Rows.Item(0).Item("A1_EPTS") = 0 And auditData.Rows.Item(0).Item("IDT") = 0 And auditData.Rows.Item(0).Item("PCARS") = 0 And auditData.Rows.Item(0).Item("EightYearRule") = 0 And auditData.Rows.Item(0).Item("A1_Other").Equals("")) Then
                        errors.Add(New ValidationItem("Audit", "Approving Authority/SAF MRR Questions", "Reason status was not validated and accurate"))
                        SignButton.Enabled = False
                    End If
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("Determination"))) Then
                    errors.Add(New ValidationItem("Audit", "Approving Authority/SAF MRR Questions", "Evidence worsened pre-service condition"))
                    SignButton.Enabled = False
                End If
            End If
            'LOD Audit (JA) Board Legal
            If (SESSION_WS_ID(refId) = 334) Then
                If (IsDBNull(auditData.Rows.Item(0).Item("LegallySufficient"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Legal", "Case legally sufficient"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("JA_ProofApplied"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Legal", "Standard of proof applied"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("JA_CorrectStandard"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Legal", "Unmistakable or Preponderance of Evidence"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("JA_ProofMet"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Legal", "Standard for proof met"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("JA_Evidence"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Legal", "Evidence of condition"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("JA_Misconduct"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Legal", "Evidence of misconduct"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("JA_FormalInvestigation"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Legal", "Formal investigation"))
                    SignButton.Enabled = False
                End If
                If (Not IsDBNull(auditData.Rows.Item(0).Item("LegallySufficient"))) Then
                    If (auditData.Rows.Item(0).Item("LegallySufficient") = 0 And auditData.Rows.Item(0).Item("JA_StandardOfProof") = 0 And auditData.Rows.Item(0).Item("JA_DeathAndMVA") = 0 And auditData.Rows.Item(0).Item("JA_FormalPolicy") = 0 And auditData.Rows.Item(0).Item("JA_AFI") = 0 And auditData.Rows.Item(0).Item("JA_Other").Equals("")) Then
                        errors.Add(New ValidationItem("Audit", "Board Legal", "Reason case not legally sufficient"))
                        SignButton.Enabled = False
                    End If
                End If
            End If
            'LOD Audit (SG)
            If (SESSION_WS_ID(refId) = 333) Then
                If (IsDBNull(auditData.Rows.Item(0).Item("MedicallyAppropriate"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Medical", "Case medically appropriate"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("SG_ProofApplied"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Medical", "Standard of proof applied"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("SG_CorrectStandard"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Medical", "Unmistakable or Preponderance of Evidence"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("SG_ProofMet"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Medical", "Standard for proof met"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("SG_Evidence"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Medical", "Evidence of condition"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("SG_Misconduct"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Medical", "Evidence of misconduct"))
                    SignButton.Enabled = False
                End If
                If (IsDBNull(auditData.Rows.Item(0).Item("SG_FormalInvestigation"))) Then
                    errors.Add(New ValidationItem("Audit", "Board Medical", "Formal investigation"))
                    SignButton.Enabled = False
                End If
                If (Not IsDBNull(auditData.Rows.Item(0).Item("MedicallyAppropriate"))) Then
                    If (auditData.Rows.Item(0).Item("MedicallyAppropriate") = 0 And auditData.Rows.Item(0).Item("SG_DX") = 0 And auditData.Rows.Item(0).Item("SG_ISupport") = 0 And auditData.Rows.Item(0).Item("SG_EPTS") = 0 And auditData.Rows.Item(0).Item("SG_Aggravation") = 0 And auditData.Rows.Item(0).Item("SG_Principles") = 0 And auditData.Rows.Item(0).Item("SG_Other").Equals("")) Then
                        errors.Add(New ValidationItem("Audit", "Board Medical", "Reason case not medically appropriate"))
                        SignButton.Enabled = False
                    End If
                End If
            End If
...
#Region "ChangeStatusAndActions"
        ...
        Public Sub SetFinalDecision(ByVal _statusIn As Integer, ByVal _statusOut As Integer)
            ...
            Select Case WorkstatusDao.GetById(_statusIn).StatusCodeType.Id
                ...
                Case LodStatusCode.LODBoardPersonnelReview
                    Dim hqaaFinding As LineOfDutyFindings
                    Dim hqbdFinding As LineOfDutyFindings
                    Dim cFinding As LineOfDutyFindings
                    hqaaFinding = LOD.FindByType(PersonnelTypes.BOARD_AA)
                ...
                Case LodStatusCode.FormalLODBoardPersonnelReview
