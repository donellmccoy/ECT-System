...
#Region "Page Methods"
        ...
        Public Sub GetForm348()
            ...
            If (LOD.Formal) AndAlso (LOD.LODInvestigation IsNot Nothing) Then
                ...
                Else
                    Description.Text = "AF Form 348 and DD Form 261"
                End If
            End If
        End Sub

        Public Sub MemoRepeater_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles MemoRepeater.ItemCommand
        ...
        Protected Function IsDeterminationMemo(ByVal templateId As Integer) As Boolean
            If (templateId = MemoType.LodFindingsILOD OrElse templateId = MemoType.LodFindingsILODDeath OrElse templateId = MemoType.LodFindingsNLOD OrElse templateId = MemoType.LodFindingsNLODDeath OrElse
                templateId = MemoType.ANGLodFindingsILOD OrElse templateId = MemoType.ANGLodFindingsILODDeath OrElse templateId = MemoType.ANGLodFindingsNLOD OrElse templateId = MemoType.ANGLodFindingsNLODDeath) Then
                Return True
            End If
        ...
        '    If (LOD.DocumentGroupId Is Nothing OrElse LOD.DocumentGroupId = 0) Then   -- 10/11/2023  Diamante Lawson :   The code currently checks if the DocumentGroupId is Nothing or 0. However, if the intent is to ensure the groupId is always greater than or equal to 0,--
        ...
        Protected Sub UpdateMemoList()
            ...
            End If
            'ForTesting88
            If (LOD.CurrentStatusCode = LodStatusCode.Complete_LODV3 Or LOD.CurrentStatusCode = LodStatusCode.Complete) Then
            ...
            Else
                AppealMemos.Add(MemoType.ANGApprovalAppeal)
                AppealMemos.Add(MemoType.ANGDisapprovalAppeal)
                AppealMemos.Add(MemoType.ANGSARC_APPEAL_APPROVED)
                AppealMemos.Add(MemoType.ANGSARC_APPEAL_DISAPPROVAL)
            End If
        ...
        Private Function GetIOAppointmentMemos(filterOutOriginalLODMemos) As IList(Of Memorandum)
            If (filterOutOriginalLODMemos) Then
                Return MemoStore.GetByRefId(RequestId).Where(Function(m) m.Deleted = False AndAlso (m.Template.Id = MemoType.LodAppointIo OrElse m.Template.Id = MemoType.ANGLodAppointIo) AndAlso Not IsOriginalLODMemo(m)).OrderBy(Function(m) m.CreatedDate).ToList()
            Else
                Return MemoStore.GetByRefId(RequestId).Where(Function(m) m.Deleted = False AndAlso (m.Template.Id = MemoType.LodAppointIo OrElse m.Template.Id = MemoType.ANGLodAppointIo)).OrderBy(Function(m) m.CreatedDate).ToList()
            End If
        End Function
            ...
            If (LODAppeal IsNot Nothing) Then
                If (LODAppeal.CurrentStatusCode = AppealStatusCode.AppealApproved OrElse LODAppeal.CurrentStatusCode = AppealStatusCode.AppealDenied) Then
                    ...
                    Dim AppealMemo = (From m In MemoStore.GetByRefId(LOD.Id)
                                      Where m.Deleted = False AndAlso (m.Template.Id = MemoType.ApprovalAppeal OrElse m.Template.Id = MemoType.DisapprovalAppeal OrElse m.Template.Id = MemoType.ANGApprovalAppeal OrElse m.Template.Id = MemoType.ANGDisapprovalAppeal)
                                      Select m
                    ...
                    ElseIf (AppealMemo.Template.Id = MemoType.DisapprovalAppeal) Then
                        AppealDescription.Text = "AFRC CV Disapproval of Appeal Memorandum"
                    ElseIf (AppealMemo.Template.Id = MemoType.ApprovalAppeal) Then
                        AppealDescription.Text = "ANG CV Approval of Appeal Memorandum"
                    ElseIf (AppealMemo.Template.Id = MemoType.DisapprovalAppeal) Then
                        AppealDescription.Text = "ANG CV Disapproval of Appeal Memorandum"
                    Else
                        AppealDescription.Text = "(unknown)"
                    End If
        ...
        Private Sub GetSARCAppealMemo()
            ...
            If (SARCAppeal IsNot Nothing) Then
                If (SARCAppeal.Status = SARCAppealWorkStatus.Approved OrElse SARCAppeal.Status = SARCAppealWorkStatus.Denied) Then
                    ...
                    Dim AppealMemo = (From m In MemoStore.GetByRefId(LOD.Id)
                                      Where m.Deleted = False AndAlso (m.Template.Id = MemoType.SARC_APPEAL_APPROVED OrElse m.Template.Id = MemoType.SARC_APPEAL_DISAPPROVAL OrElse m.Template.Id = MemoType.ANGSARC_APPEAL_APPROVED OrElse m.Template.Id = MemoType.ANGSARC_APPEAL_DISAPPROVAL)
                                      Select m
                                      Order By m.CreatedDate Descending).FirstOrDefault
                    ...
                    ElseIf (AppealMemo.Template.Id = MemoType.SARC_APPEAL_DISAPPROVAL) Then
                        SARCAppealDescription.Text = "AFRC CV Disapproval of SARC Appeal Memorandum"
                    ElseIf (AppealMemo.Template.Id = MemoType.ANGSARC_APPEAL_APPROVED) Then
                        SARCAppealDescription.Text = "ANG CV Approval of SARC Appeal Memorandum"
                    ElseIf (AppealMemo.Template.Id = MemoType.ANGSARC_APPEAL_DISAPPROVAL) Then
                        SARCAppealDescription.Text = "ANG CV Disapproval of SARC Appeal Memorandum"
                    Else
                        SARCAppealDescription.Text = "(unknown)"
                    End If
        ...
        Private Sub InitMemoCreation()
            Dim template As MemoTemplate = Nothing ' Explicitly set it to Nothing
            ...
            Dim templateIds As New List(Of Integer)
            ...
            Select Case LOD.CurrentStatusCode
                Case LodStatusCode.NotifyFormalInvestigator
                    If (LOD.AppointedIO IsNot Nothing) Then
                        ' Grab IO appointment memos
                        Dim ioTemplates = From t In templates
                                          Where t.Id = MemoType.LodAppointIo OrElse t.Id = MemoType.ANGLodAppointIo
                                          Select t

                        For Each template In ioTemplates
                ...
                Case LodStatusCode.Complete_LODV3
                    If Not (CheckPostCompletionData()) Then Exit Select

                    ' Determine Findings
                    Dim findings As String = LOD.LodFindings.ElementAt(1).Description
                    Dim isDeathInvolved As Boolean = (LOD.LODMedical.DeathInvolved = "Yes")

                    ' 🔹 **ILOD Findings**
                    If LOD.FinalFindings = ALOD.Core.Utils.Finding.In_Line_Of_Duty OrElse findings.Equals("In Line of Duty") Then
                        If isDeathInvolved Then
                            templateIds.AddRange(From t In templates
                                                 Where t.Id = MemoType.LodFindingsILODDeath OrElse t.Id = MemoType.ANGLodFindingsILODDeath
                                                 Select t.Id)
                        Else
                            templateIds.AddRange(From t In templates
                                                 Where t.Id = MemoType.LodFindingsILOD OrElse t.Id = MemoType.ANGLodFindingsILOD
                                                 Select t.Id)
                        End If
                    End If

                    ' 🔹 **NLOD Findings (Due to Misconduct or Not)**
                    If LOD.FinalFindings = ALOD.Core.Utils.Finding.Nlod_Due_To_Own_Misconduct OrElse
                            findings.Equals("Not ILOD - Due To Own Misconduct") OrElse
                            LOD.FinalFindings = ALOD.Core.Utils.Finding.Nlod_Not_Due_To_OwnMisconduct OrElse
                            findings.Equals("Not ILOD - Not Due To Own Misconduct") Then

                        If isDeathInvolved Then
                            templateIds.AddRange(From t In templates
                                                 Where t.Id = MemoType.LodFindingsNLODDeath OrElse t.Id = MemoType.ANGLodFindingsNLODDeath
                                                 Select t.Id)
                        Else
                            templateIds.AddRange(From t In templates
                                                 Where t.Id = MemoType.LodFindingsNLOD OrElse t.Id = MemoType.ANGLodFindingsNLOD
                                                 Select t.Id)
                        End If
                    End If

                    If templateIds.Count > 0 Then isDeterminationTemplate = True
            End Select

            ' Show/Hide Create Memo button
            If templateIds.Count = 0 Then
                CreateMemo.Visible = False
            Else
                CreateMemo.Visible = True
                Dim templateIdString = String.Join(",", templateIds)
                CreateMemo.Attributes.Add("onclick", $"showEditor(0,'{templateIdString}',{isDeterminationTemplate.ToString().ToLower()});")
            End If
        End Sub
