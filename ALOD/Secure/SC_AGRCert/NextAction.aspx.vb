...
Namespace Web.Special_Case.AGR
    ...
    Partial Class Secure_sc_agr_NextAction
        ...
        Private boardStages As Short() = New Short() {
            CShort(SpecCaseAGRWorkStatus.HqInitiateCase),
            CShort(SpecCaseAGRWorkStatus.PackageReview),
            CShort(SpecCaseAGRWorkStatus.MedicalReview),
            CShort(SpecCaseAGRWorkStatus.MedicalOfficerReview),
            CShort(SpecCaseAGRWorkStatus.FinalReview)}

        Private CANCELLATION_REASON As String = "Case Cancelled By "
        Private errors As New List(Of ValidationItem)
        Private sc As SC_AGRCert
        Private separatorTemplate As System.Web.UI.CompiledTemplateBuilder

        Private unitStages As Short() = New Short() {
            CShort(SpecCaseAGRWorkStatus.InitiateCase)}
...
#End Region
        ...
        Protected Function GetStatusInfo(ByVal statusCodeOut As Integer) As Tuple(Of Integer, String)
            ...
            Select Case statusCodeOut
                Case SpecCaseAGRWorkStatus.InitiateCase, SpecCaseAGRWorkStatus.FinalReview
                    sentTo = "Medical Technician"
                    sentToId = UserGroups.MedicalTechnician
                Case SpecCaseAGRWorkStatus.PackageReview, SpecCaseAGRWorkStatus.FinalReview
                    sentTo = "HQ AFRC Technician"
                    sentToId = UserGroups.AFRCHQTechnician
                Case SpecCaseAGRWorkStatus.HqInitiateCase
                    sentTo = "HQ AFRC Technician"
                    sentToId = UserGroups.AFRCHQTechnician
                ...
                Case SpecCaseAGRWorkStatus.MedicalOfficerReview
                    sentTo = "Medical Officer Reviewer"
        ...
        Private Sub ShowRLB()
            Dim access As ALOD.Core.Domain.Workflow.PageAccessType = SectionList(AGRSectionNames.AGR_RLB.ToString())
...
#Region "ChangeStatusAndActions"
        ...
        Public Sub AddEmails(ByRef emails As MailManager, emailService As EmailService, action As WorkflowOptionAction, statusOut As Integer)
            ...
            If (statusOut = SpecCaseAGRWorkStatus.Approved Or statusOut = SpecCaseAGRWorkStatus.Denied) Then
        ...
        Public Sub ApplyActions(ByVal optionId As Integer, ByVal statusIn As Integer, ByVal statusOut As Integer)
            ...
            emails.SetField("MED_OFF_COMMENTS", Server.HtmlDecode(SpecCase.med_off_approval_comment))
            emails.SetField("RETURN_COMMENTS", Server.HtmlEncode(CommentsTextBox.Text))

            If (sc.ExpirationDate.HasValue) Then
                emails.SetField("EXPIRATION_DATE", sc.ExpirationDate.Value.ToString(DATE_FORMAT))
            End If
            'If (Not String.IsNullOrEmpty(sc.ICD9Diagnosis)) Then
            '    emails.SetField("DIAGNOSIS", sc.ICD9Diagnosis)
            'End If

            emails.SetField("SIGNATURE", String.Format("{0} {1}, {2} {3}", user.Rank.Rank, user.LastName, user.FirstName, user.MiddleName))
        ...
        Public Sub ReturnBack(ByVal statusIn As Integer, ByVal statusOut As Integer, ByVal ReturnFlag As Boolean)
            ...
            If (rwRec IsNot Nothing AndAlso rwRec.DateSentBack Is Nothing) Then
                ...
                ElseIf (statusCodeOut = SpecCaseAGRWorkStatus.Cancel) Then           'Case was canceled during Return
                    rwRec.DateSentBack = DateTime.Now
                ...
                ElseIf (statusCodeOut = SpecCaseAGRWorkStatus.Approved OrElse statusCodeOut = SpecCaseAGRWorkStatus.Denied) Then            'Case was completed during Return
