...
Namespace Web.Special_Case.RW
    ...
    Partial Class Secure_sc_rw_NextAction
...
#Region "Fields..."
        ...
        Private _returnDao As IReturnDao
        Private _rwoaDao As IRwoaDao
        ...
        Private boardStages As Short() = New Short() {
            CShort(SpecCaseRWStatusCode.HQAFRCTechInitiateCase),
            CShort(SpecCaseRWStatusCode.MedicalReview),
            CShort(SpecCaseRWStatusCode.SeniorMedicalReview),
            CShort(SpecCaseRWStatusCode.FinalReview),
            CShort(SpecCaseRWStatusCode.PackageReview)}
        ...
        Private unitStages As Short() = New Short() {
            CShort(SpecCaseRWStatusCode.MedTechInitiateCase)}
...
#Region "Properties..."
        ...
        Protected ReadOnly Property ReminderEmailsDao() As IReminderEmailDao
            Get
        ...
        Protected ReadOnly Property ReturnDao() As IReturnDao
            Get
                If (_returnDao Is Nothing) Then
                    _returnDao = DaoFactory.GetReturnDao()
                End If
        ...
        Protected Property RLBEnabled() As Boolean
            Get
        ...
        Protected Property RLBVisible() As Boolean
            Get
        ...
        Protected ReadOnly Property RWOADao() As IRwoaDao
            Get
                If (_rwoaDao Is Nothing) Then
                    _rwoaDao = DaoFactory.GetRwoaDao()
                End If
...
#Region "Page Methods..."
        ...
        Public Sub AddEmails(ByRef emails As MailManager, emailService As EmailService, action As WorkflowOptionAction, statusOut As Integer)
            ...
            If (statusOut = SpecCaseRWWorkStatus.RTD OrElse statusOut = SpecCaseRWWorkStatus.Disqualified OrElse statusOut = SpecCaseRWWorkStatus.AdminLOD) Then
        ...
        Public Sub ChangeStatus(optionId As Integer, newStatus As Integer, text As String, comments As String)
            ...
            ReminderEmailsDao.ReminderEmailUpdateStatusChange(statusIn, newStatus, SpecCase.CaseId, "SC")
        End Sub
            ...
            If (rwRec IsNot Nothing AndAlso rwRec.DateSentBack Is Nothing) Then
                If (rwRec.WorkStatusFrom = statusOut) Then                      'Case made it back to where the Return came from
                    ...
                    Else                                                        'Case was rerouted
                        ...
                        rwRec.DateSentBack = DateTime.Now
                    End If
                ElseIf (statusCodeOut = SpecCaseRWStatusCode.Cancelled) Then           'Case was canceled during Return
                    rwRec.DateSentBack = DateTime.Now
                    If (String.IsNullOrEmpty(rwRec.CommentsBackToSender)) Then
                        ...
                        rwRec.rerouting = 0
                    End If
                ElseIf (statusCodeOut = SpecCaseRWStatusCode.RTD OrElse statusCodeOut = SpecCaseRWStatusCode.Disqualified OrElse statusOut = SpecCaseRWWorkStatus.AdminLOD) Then            'Case was completed during Return
                    If (String.IsNullOrEmpty(rwRec.CommentsBackToSender)) Then
                        rwRec.DateSentBack = DateTime.Now
        ...
        Protected Function GetStatusInfo(ByVal statusCodeOut As Integer) As Tuple(Of Integer, String)
            ...
            Select Case statusCodeOut
                Case SpecCaseRWStatusCode.MedTechInitiateCase
                    sentTo = "Medical Technician"
                    sentToId = UserGroups.MedicalTechnician
                Case SpecCaseRWStatusCode.HQAFRCTechInitiateCase, SpecCaseRWStatusCode.PackageReview, SpecCaseRSStatusCode.FinalReview
                    sentTo = "HQ Tech"
                    sentToId = UserGroups.AFRCHQTechnician
                Case SpecCaseRWStatusCode.MedicalReview
                    sentTo = "Board Medical"
                    sentToId = UserGroups.BoardMedical
                Case SpecCaseRWStatusCode.SeniorMedicalReview
            ...
            Return (New Tuple(Of Integer, String)(sentToId, sentTo))
        End Function
        ...
        Private Sub ShowRLB()
            Dim access As ALOD.Core.Domain.Workflow.PageAccessType = SectionList(RWSectionNames.RW_RLB.ToString())
            Dim ws As IList(Of WorkStatusTracking) = WorkFlowService.GetWorkStatusTracking(SpecCase.Id, ModuleType)
