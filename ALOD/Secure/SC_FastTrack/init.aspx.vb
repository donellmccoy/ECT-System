...
Namespace Web.Special_Case.IRILO
    ...
    Partial Class Secure_FastTrack_init
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            Dim userAccess As PageAccess.AccessLevel = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCaseFT)

            Utility.VerifyUserAccess(userAccess, Resources.Messages.ERROR_NO_ACCESS, Resources._Global.StartPage)

            Dim scFastTrack As SC_FastTrack = MasterPage.LoadCase(refId)

            Session("refId") = refId

            Utility.UpdateCaseLock(userAccess, refId, ModuleType.SpecCaseFT)

            MasterPage.InitPageAccess(scFastTrack)

            Dim startPage As String = MasterPage.GetStartPageTitle(scFastTrack.Status)

            'remove the scWWD from the rDao so no changes are persisted back
            SCDao.Evict(scFastTrack)
