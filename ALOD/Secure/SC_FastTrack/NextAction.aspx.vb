...
Namespace Web.Special_Case.IRILO
    ...
    Partial Class Secure_sc_FastTrack_NextAction
        ...
        Private boardStages As Short() = New Short() {
            CShort(SpecCaseFTStatusCode.PackageReview),
            CShort(SpecCaseFTStatusCode.MedicalReview),
            CShort(SpecCaseFTStatusCode.SeniorMedicalReview),
            CShort(SpecCaseFTStatusCode.FinalReview),
            CShort(SpecCaseFTStatusCode.AdminLOD)}
        ...
        Private unitStages As Short() = New Short() {
            CShort(SpecCaseFTStatusCode.InitiateCase)}
...
#End Region
        ...
        Protected Function GetStatusInfo(ByVal statusCodeOut As Integer) As Tuple(Of Integer, String)
            ...
            Select Case statusCodeOut
                Case SpecCaseFTStatusCode.InitiateCase
                    sentTo = "Medical Technician"
                    sentToId = UserGroups.MedicalTechnician
                Case SpecCaseFTStatusCode.PackageReview, SpecCaseFTStatusCode.FinalReview, SpecCaseFTStatusCode.AdminLOD, SpecCaseFTStatusCode.IMRPHolding
                    sentTo = "HQ Tech"
                    sentToId = UserGroups.AFRCHQTechnician
                Case SpecCaseFTStatusCode.MedicalReview
                    sentTo = "Board Medical"
                    sentToId = UserGroups.BoardMedical
                Case SpecCaseFTStatusCode.SeniorMedicalReview
        ...
        Protected Sub SignButton_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles SignButton.Click
            'TODO
            'We need to apply fixes for CR-016 and BG-022
            'Changing "RWOA" to "RFA" broke the following code blocks
            Dim workOption = GetWorkOption()
        ...
        Private Sub InitNextActionOptions()
            ...
            '        '1981 1980 1979
            '        If o.Id = 1979 OrElse o.Id = 1980 OrElse o.Id = 1981 Then
            '            o.OptionValid = False
            '            o.OptionVisible = False
            '        End If
        ...
        Private Sub ShowRLB()
            Dim access As ALOD.Core.Domain.Workflow.PageAccessType = SectionList(FTSectionNames.FT_RLB.ToString())
...
#Region "ChangeStatusAndActions"
        ...
        Public Sub AddEmails(ByRef emails As MailManager, emailService As EmailService, action As WorkflowOptionAction, statusOut As Integer)
            ...
            If (statusOut = SpecCaseFTWorkStatus.RTD Or statusOut = SpecCaseFTWorkStatus.Disqualified) Then
        ...
        Public Sub ReturnBack(ByVal statusIn As Integer, ByVal statusOut As Integer, ByVal ReturnFlag As Boolean)
            ...
            If (rwRec IsNot Nothing AndAlso rwRec.DateSentBack Is Nothing) Then
                ...
                ElseIf (statusCodeOut = SpecCaseFTStatusCode.Cancelled) Then           'Case was canceled during Return
                    rwRec.DateSentBack = DateTime.Now
                    If (String.IsNullOrEmpty(rwRec.CommentsBackToSender)) Then
                ...
                ElseIf (statusCodeOut = SpecCaseFTStatusCode.Disqualified OrElse statusCodeOut = SpecCaseFTStatusCode.RTD) Then            'Case was completed during Return
                    rwRec.DateSentBack = DateTime.Now
                    If (String.IsNullOrEmpty(rwRec.CommentsBackToSender)) Then
