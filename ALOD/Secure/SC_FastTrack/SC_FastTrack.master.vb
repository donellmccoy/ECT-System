...
Namespace Web.Special_Case.IRILO
    ...
    Partial Class SC_FastTrackMaster
        Inherits System.Web.UI.MasterPage

        Private _dao As ISpecialCaseDAO
        Private _sc As SC_FastTrack = Nothing
        ...
        Public Function ConstructSeniorMedicalReviewerTabVisibilityArgs() As SeniorMedicalReviewerTabVisibilityArgs
            ...
            workStatuses.Add(SpecCaseFTWorkStatus.SeniorMedicalReview)

            args.RefId = SpecCase.Id
            args.ModuleId = SpecCase.moduleId
            args.WorkStatusIds = workStatuses
            args.TabTitle = "IR Senior Med"
            args.Steps = Navigator.Steps

            Return args
        End Function

        Public Function GetStartPageTitle(ByVal workStatus As SpecCaseFTWorkStatus) As String
            Dim startPage As String = "IR Member"

            Select Case workStatus
                Case SpecCaseFTWorkStatus.InitiateCase
                    If (GroupId = UserGroups.MedicalTechnician) Then
                        startPage = "IR Med Tech"
                    End If

                Case SpecCaseFTWorkStatus.MedicalReview
                    If (GroupId = UserGroups.BoardMedical) Then
                        startPage = "IR Med Off"
                    End If
        ...
        Public Sub InitPageAccess(ByVal specCase As SC_FastTrack)
        ...
        Public Function LoadCase(ByVal refId As Integer) As SC_FastTrack
            Dim scFastTrack As SC_FastTrack = Nothing
            ...
            Try
                If _scId <> refId Then
                    ...
                    _scId = refId
                End If
                scFastTrack = SpecCase
            Catch ex As Exception
                'we failed to load the case, nothing we can do
                ALOD.Logging.LogManager.LogError("scDao Error loading Case of Type (" + WorkflowDao.GetCaseType(ModuleType.SpecCaseFT) + ") with refId: " + refId.ToString())
            ...
            If (scFastTrack Is Nothing) Then
                'we failed to load the case, nothing we can do
                ALOD.Logging.LogManager.LogError("NULL Fast Track Error loading " + WorkflowDao.GetCaseType(ModuleType.SpecCaseFT) + " with requestId: " + refId.ToString())
                SetErrorMessage(Resources.Messages.ERROR_LOADING_CASE)
                Response.Redirect(Resources._Global.StartPage, True)
            End If
        ...
        Private Sub InitNavigator(ByVal refId As Integer)
            ...
            userAccess = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCaseFT)

            Utility.VerifyUserAccess(userAccess, Resources.Messages.ERROR_NO_ACCESS, Resources._Global.StartPage)

            sc = LoadCase(refId)

            Utility.UpdateCaseLock(userAccess, refId, ModuleType.SpecCaseFT)
            InitPageAccess(sc)
