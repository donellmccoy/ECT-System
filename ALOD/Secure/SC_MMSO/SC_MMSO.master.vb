...
Namespace Web.Special_Case.MMSO
    ...
    Partial Class SC_MMSOMaster
        Inherits System.Web.UI.MasterPage

        Private _dao As ISpecialCaseDAO
        Private _sc As SC_MMSO = Nothing
        ...
        Public Function GetStartPageTitle(ByVal workStatus As SpecCaseMMSOWorkStatus) As String
            Dim startPage As String = "MM Member"

            Select Case workStatus
                Case SpecCaseMMSOWorkStatus.InitiateCase
                    If (GroupId = UserGroups.MedicalOfficer Or GroupId = UserGroups.MedicalTechnician) Then
                        startPage = "Pre-authorization Request"
                    End If

                Case SpecCaseMMSOWorkStatus.SMRInput
                    If (GroupId = UserGroups.SeniorMedicalReviewer) Then
                        startPage = "Pre-authorization Request"
                    End If

                Case SpecCaseMMSOWorkStatus.ContractRepReview
                    If GroupId = UserGroups.MMSOContractRepresentative Or GroupId = UserGroups.MMSONurse Or GroupId = UserGroups.MMSOServicePOS Then
                        startPage = "MM Next Action"
                    End If

                Case SpecCaseMMSOWorkStatus.NurseReview
                    If GroupId = UserGroups.MMSONurse Then
                        startPage = "MM Next Action"
                    End If

                Case SpecCaseMMSOWorkStatus.SPOCReview
                    If GroupId = UserGroups.MMSOServicePOS Then
                        startPage = "MM Next Action"
                    End If
        ...
        Public Sub InitPageAccess(ByVal specCase As SC_MMSO)
        ...
        Public Function LoadCase(ByVal refId As Integer) As SC_MMSO
            Dim scMMSO As SC_MMSO = Nothing
            ...
            If (scMMSO Is Nothing) Then
                'we failed to load the case, nothing we can do
                ALOD.Logging.LogManager.LogError("NULL MMSO Error loading MMSO with requestId: " + refId.ToString())
                SetErrorMessage(Resources.Messages.ERROR_LOADING_CASE)
                Response.Redirect(Resources._Global.StartPage, True)
            End If
