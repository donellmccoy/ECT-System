...
Namespace Web.Special_Case.BMT
    ...
    Partial Class Secure_BMT_init
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            Dim userAccess As PageAccess.AccessLevel = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCaseBMT)

            Utility.VerifyUserAccess(userAccess, Resources.Messages.ERROR_NO_ACCESS, Resources._Global.StartPage)

            Dim scBMT As SC_BMT = MasterPage.LoadCase(refId)

            Session("refId") = refId

            Utility.UpdateCaseLock(userAccess, refId, ModuleType.SpecCaseBMT)

            MasterPage.InitPageAccess(scBMT)

            Dim startPage As String = MasterPage.GetStartPageTitle(scBMT.Status)

            MasterPage.SetSubWorkflowType(scBMT)

            'remove the scbmt from the rDao so no changes are persisted back
            SCDao.Evict(scBMT)
