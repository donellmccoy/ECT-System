...
Namespace Web.Special_Case.MO
    ...
    Partial Class SC_MOMaster
        Inherits System.Web.UI.MasterPage

        Private _dao As ISpecialCaseDAO
        Private _sc As SC_MO = Nothing
        ...
        Public Function ConstructSeniorMedicalReviewerTabVisibilityArgs() As SeniorMedicalReviewerTabVisibilityArgs
            ...
            workStatuses.Add(SpecCaseMOWorkStatus.SeniorMedicalReview)

            args.RefId = SpecCase.Id
            args.ModuleId = SpecCase.moduleId
            args.WorkStatusIds = workStatuses
            args.TabTitle = "MO Senior Med"
            args.Steps = Navigator.Steps

            Return args
        End Function

        Public Function GetStartPageTitle(ByVal workStatus As SpecCaseMOWorkStatus) As String
            Dim startPage As String = "MO Member"

            Select Case workStatus
                Case SpecCaseMOWorkStatus.MedTechInitiateCase
                    If (GroupId = UserGroups.MedicalTechnician) Then
                        startPage = "MO Med Tech"
                    End If

                Case SpecCaseMOWorkStatus.HQAFRCTechInitiateCase
                    If (GroupId = UserGroups.AFRCHQTechnician) Then
                        startPage = "MO Med Tech"
                    End If

                Case SpecCaseMOWorkStatus.MedicalReview
                    If (GroupId = UserGroups.BoardMedical) Then
                        startPage = "MO Board Med"
                    End If
        ...
        Public Sub InitPageAccess(ByVal specCase As SC_MO)
        ...
        Public Function LoadCase(ByVal refId As Integer) As SC_MO
            Dim scMO As SC_MO = Nothing
            ...
            If (scMO Is Nothing) Then
                'we failed to load the case, nothing we can do
                ALOD.Logging.LogManager.LogError("NULL Modifications Error loading Modification with requestId: " + refId.ToString())
                SetErrorMessage(Resources.Messages.ERROR_LOADING_CASE)
                Response.Redirect(Resources._Global.StartPage, True)
            End If
        ...
        Private Sub InitNavigator(ByVal refId As Integer)
            ...
            Dim scMO As New SC_MO

            Session("refId") = refId

            userAccess = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCaseMO)
