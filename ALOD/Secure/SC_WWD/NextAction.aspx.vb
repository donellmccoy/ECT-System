...
Namespace Web.Special_Case.WWD
    ...
    Partial Class Secure_sc_WWD_NextAction
        ...
        Private boardStages As Short() = New Short() {
            CShort(SpecCaseWWDStatusCode.PackageReview),
            CShort(SpecCaseWWDStatusCode.MedicalReview),
            CShort(SpecCaseWWDStatusCode.SeniorMedicalReview),
            CShort(SpecCaseWWDStatusCode.FinalReview),
            CShort(SpecCaseWWDStatusCode.MedicalReviewSAF),
            CShort(SpecCaseWWDStatusCode.SeniorMedicalReviewSAF),
            CShort(SpecCaseWWDStatusCode.FinalReviewSAF),
            CShort(SpecCaseWWDStatusCode.AdminLOD),
            CShort(SpecCaseWWDStatusCode.AwaitingSAF),
            CShort(SpecCaseWWDStatusCode.IPEBDisposition),
            CShort(SpecCaseWWDStatusCode.FPEBDisposition),
            CShort(SpecCaseWWDStatusCode.MedicalReviewIPEB),
            CShort(SpecCaseWWDStatusCode.SeniorMedicalReviewIPEB),
            CShort(SpecCaseWWDStatusCode.FinalReviewIPEB),
            CShort(SpecCaseWWDStatusCode.MedicalReviewFPEB),
            CShort(SpecCaseWWDStatusCode.SeniorMedicalReviewFPEB),
            CShort(SpecCaseWWDStatusCode.FinalReviewFPEB)}
        ...
        Private unitStages As Short() = New Short() {
            CShort(SpecCaseWWDStatusCode.InitiateCase)}
...
#End Region
        ...
        Protected Function GetStatusInfo(ByVal statusCodeOut As Integer) As Tuple(Of Integer, String)
            ...
            Select Case statusCodeOut
                Case SpecCaseWWDStatusCode.InitiateCase
                    sentTo = "Medical Technician"
                    sentToId = UserGroups.MedicalTechnician
                Case SpecCaseWWDStatusCode.PackageReview, SpecCaseWWDStatusCode.FinalReview, SpecCaseWWDStatusCode.FinalReviewSAF, SpecCaseWWDStatusCode.AwaitingSAF,
                     SpecCaseWWDStatusCode.IPEBDisposition, SpecCaseWWDStatusCode.FPEBDisposition, SpecCaseWWDStatusCode.FinalReviewIPEB, SpecCaseWWDStatusCode.FinalReviewFPEB
                    sentTo = "HQ AFRC Technician"
                    sentToId = UserGroups.AFRCHQTechnician
                Case SpecCaseWWDStatusCode.MedicalReview, SpecCaseWWDStatusCode.MedicalReviewSAF, SpecCaseWWDStatusCode.MedicalReviewIPEB, SpecCaseWWDStatusCode.MedicalReviewFPEB
                    sentTo = "Board Medical"
                    sentToId = UserGroups.BoardMedical
                Case SpecCaseWWDStatusCode.SeniorMedicalReview, SpecCaseWWDStatusCode.SeniorMedicalReviewSAF, SpecCaseWWDStatusCode.SeniorMedicalReviewIPEB, SpecCaseWWDStatusCode.SeniorMedicalReviewFPEB
        ...
        Private Sub InitNextActionOptions()
            ...
            '        '266 = Disable “Return to Senior Medical Reviewer.” 'OrElse o.Id = 944 turned back on via Gary 10/2/18
            '        If o.Id = 157 OrElse o.Id = 195 OrElse o.Id = 158 OrElse o.Id = 177 OrElse o.Id = 266 OrElse o.Id = 403 OrElse o.Id = 161 OrElse o.Id = 940 OrElse o.Id = 941 OrElse o.Id = 510 Then
            '            o.OptionValid = False
        ...
        ''' <summary>
        ''' WWD DQ Workflow.
        ''' Board Medical makes Disqualified decision.
        ''' HQ AFRC Tech needs to upload the SAF Letter in the Final Review Step.
        ''' We need to set the Begin Date of the Traking Item to the date the SAF Letter was uploaded.
        ''' 1) We need to look for the specfic Tracking Item. Last Item that has the WD Final Review step.
        ''' 2) Update the item {Begin/Start Date} with the SAF Uploaded date.
        ''' </summary>
        ''' <remarks>This is only for the WWD DQ Workflow.</remarks>
        Private Sub SetSAFUploadDate()

            Dim data As WorkStatusTracking = (From List In LookupService.GetWorkStatusTracking(RefId, ModuleType)
                                              Where List.WorkflowStatus.Id = SpecCaseWWDWorkStatus.FinalReview
                                              Order By List.WorkflowStatus.Id Descending
                                              Select List).FirstOrDefault()

            'Adding -1 day for testing purposes.
            'data.StartDate = SpecCase.SAFLetterUploadDate.Value.AddDays(-1)
            If SpecCase.SAFLetterUploadDate.HasValue Then
                data.StartDate = SpecCase.SAFLetterUploadDate.Value
            End If

            Dim workStatusTrackingDao As IWorkStatusTrackingDao = New NHibernateDaoFactory().GetWorkStatusTrackingDao()
            'Save
            workStatusTrackingDao.SaveOrUpdate(data)
            workStatusTrackingDao.CommitChanges()

        End Sub

        Private Sub ShowRLB()
            Dim access As ALOD.Core.Domain.Workflow.PageAccessType = SectionList(WDSectionNames.WD_RLB.ToString())
...
#Region "ChangeStatusAndActions"
        ...
        Public Sub AddEmails(ByRef emails As MailManager, emailService As EmailService, action As WorkflowOptionAction, statusOut As Integer)
            ...
            If (statusOut = SpecCaseWWDWorkStatus.Approved Or statusOut = SpecCaseWWDWorkStatus.Denied Or statusOut = SpecCaseWWDWorkStatus.AwaitingSAF) Then
        ...
        Public Sub ApplyActions(ByVal optionId As Integer, ByVal statusIn As Integer, ByVal statusOut As Integer)
            ...
            For Each actn As WorkflowOptionAction In optn.ActionList
                ...
                Select Case actn.ActionType.Id
                    ...
                    Case WorkflowActionType.SetSAFUploadDate
                        SetSAFUploadDate()
        ...
        Public Sub ReturnBack(ByVal statusIn As Integer, ByVal statusOut As Integer, ByVal ReturnFlag As Boolean)
            ...
            If (rwRec IsNot Nothing AndAlso rwRec.DateSentBack Is Nothing) Then
                ...
                ElseIf (statusCodeOut = SpecCaseWWDStatusCode.Cancelled) Then           'Case was canceled during Return
                    rwRec.DateSentBack = DateTime.Now
                ...
                ElseIf (statusCodeOut = SpecCaseWWDStatusCode.Approved OrElse statusCodeOut = SpecCaseWWDStatusCode.Denied) Then            'Case was completed during Return
