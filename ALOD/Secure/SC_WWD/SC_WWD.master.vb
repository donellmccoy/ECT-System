...
Namespace Web.Special_Case.WWD
    ...
    Partial Class SC_WWDMaster
        Inherits System.Web.UI.MasterPage

        Private _dao As ISpecialCaseDAO
        Private _sc As SC_WWD = Nothing
        ...
        Public Function ConstructSeniorMedicalReviewerTabVisibilityArgs() As SeniorMedicalReviewerTabVisibilityArgs
            ...
            workStatuses.Add(SpecCaseWWDWorkStatus.SeniorMedicalReview)
            workStatuses.Add(SpecCaseWWDWorkStatus.SeniorMedicalReviewSAF)
            workStatuses.Add(SpecCaseWWDWorkStatus.SeniorMedicalReviewIPEB)
            workStatuses.Add(SpecCaseWWDWorkStatus.SeniorMedicalReviewFPEB)

            args.RefId = SpecCase.Id
            args.ModuleId = SpecCase.moduleId
            args.WorkStatusIds = workStatuses
            args.TabTitle = "WD Senior Med"
            args.Steps = Navigator.Steps

            Return args
        End Function

        Public Function GetStartPageTitle(ByVal workStatus As SpecCaseWWDWorkStatus) As String
            Dim startPage As String = "WD Member"

            Select Case workStatus
                Case SpecCaseWWDWorkStatus.InitiateCase
                    If (GroupId = UserGroups.MedicalTechnician) Then
                        startPage = "WD Med Tech"
                    End If

                Case SpecCaseWWDWorkStatus.MedicalReview
                    If (GroupId = UserGroups.BoardMedical) Then
                        startPage = "WD Board Med"
                    End If
        ...
        Public Sub InitPageAccess(ByVal specCase As SC_WWD)
        ...
        Public Function LoadCase(ByVal refId As Integer) As SC_WWD
            Dim scWWD As SC_WWD = Nothing
            ...
            If (scWWD Is Nothing) Then
                'we failed to load the case, nothing we can do
                ALOD.Logging.LogManager.LogError("NULL World Wide Duty Error loading World Wide Duty with requestId: " + refId.ToString())
                SetErrorMessage(Resources.Messages.ERROR_LOADING_CASE)
                Response.Redirect(Resources._Global.StartPage, True)
            End If
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Session(SESSIONKEY_COMPO) = "6" Then
                ModHeader.ModuleHeader = "WD"
            Else
                ModHeader.ModuleHeader = "NDDES"
                ModHeader.sKeyWord = "WD"
                ModHeader.sReplacement = "NDDES"

                For Each liS As ALODWebUtility.TabNavigation.TabItem In Navigator.Steps
                    liS.Title = liS.Title.Replace("WD", "NDDES")
                Next
            End If
        ...
        Private Sub InitNavigator(ByVal refId As Integer)
            ...
            If Session(SESSIONKEY_COMPO) = "5" Then
                Navigator.ShowStep("WD HQ AFRC Tech", False)
                'Navigator.RenameAllSteps("WD", "NDDES")
            End If

            userAccess = SCDao.GetUserAccess(SESSION_USER_ID, refId, ModuleType.SpecCaseWWD)
