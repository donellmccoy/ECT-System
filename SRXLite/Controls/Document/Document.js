//Type.registerNamespace('ScriptSvc');

//Set body events
document.body.onload = DocumentOnLoad;
...
function element(id) {
	return document.getElementById(_image.getAttribute("parentid") +"_"+ id);
}
	...
	if (pages.length>0) {
		...
		for (var i=0; i<pages.length; i++) {
			...
			if (imgSrc) { //Fit image to window
				w = img.parentElement.scrollWidth - 30; //divContent
				img.style.width = w;
				img.src = imgSrc + w;
...
function CurrentImage() {
	...
	this.sourceHeight = function(){ return parseInt(getQueryString(_image.src,"h")) };
	this.sourceWidth = function(){ return parseInt(getQueryString(_image.src,"w")) };
}
*/

function getDefaultPageWidth() {
	//return document.documentElement.clientWidth - 30;
	//return element("divContent").scrollWidth - 30;
	return document.documentElement.clientWidth - element("divThumbnails").scrollWidth - 40;
}
function getMode() {
	return _image.parentElement.getAttribute("mode");
}
function isThumbnailMode() {
	return getMode()=="Thumbnail";
}
...
function DisplayMenu(e) {
	...
	var menuWidth = (_menu.scrollWidth==0)? 201 : _menu.scrollWidth+3;
	var menuHeight = (_menu.scrollHeight==0)? 218 : _menu.scrollHeight+3;
...
//		var service = ScriptSvc.DocumentScriptService;
//		//alert(ScriptSvc);
//		service.DeletePage(getImageID(),OnDeleteSuccess,OnDeleteFailed);
//	}
//}
function OnDeleteSuccess(result) {
	var content = element("divContent");
	content.removeChild(_image);
	_image = null;
...
function CheckHotKeys(e) {
	...
	switch(e.keyCode) {
		...
		//case 80: if (e.ctrlKey) Print(); break; // Ctrl+P
