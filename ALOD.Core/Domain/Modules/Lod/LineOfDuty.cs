...
{
    [Serializable]
    public class LineOfDuty : Entity, ITrackingSource
    {
        ...
        public virtual string TheCompo { get; set; }
        ...
        {
            if (!WorkflowStatus.StatusCodeType.IsFinal || !WorkflowStatus.StatusCodeType.Description.Equals("Complete (Pilot)"))
                if (!WorkflowStatus.StatusCodeType.Description.Equals("Complete"))
                    return false;
            ...
            if (memberNotified
                && HasPostCompletionDigitalSignatureBeenGenerated(daoFactory)
                && (AFRCHasDeterminationMemo(daoFactory) || ANGHasDeterminationMemo(daoFactory)))
                return true;
        ...
        public LineOfDuty()
        {
            ...
            Active.Add(DocumentType.PCARS.ToString(), false);
        ...
        public virtual bool AFRCNotificationMemoCreated(IMemoDao memoDao, string Compo)
        {
            TheCompo = Compo;
            ...
            if ((FinalFindings == (byte)ALOD.Core.Utils.Finding.In_Line_Of_Duty) || (FinalFindings == (byte)ALOD.Core.Utils.Finding.Epts_Service_Aggravated) || LodFindings.ElementAt(1).Description.Equals("In Line of Duty"))
            {
        ...
        public virtual bool ANGNotificationMemoCreated(IMemoDao memoDao, string Compo)
        {
            TheCompo = Compo;
            ...
            if ((FinalFindings == (byte)ALOD.Core.Utils.Finding.In_Line_Of_Duty) || (FinalFindings == (byte)ALOD.Core.Utils.Finding.Epts_Service_Aggravated) || LodFindings.ElementAt(1).Description.Equals("In Line of Duty"))
            {
                if (LODMedical.DeathInvolved == "Yes")
                {
                    notificationType = (byte)MemoType.ANGLodFindingsILODDeath;
                }
                else
                {
                    notificationType = (byte)MemoType.ANGLodFindingsILOD;
                }
            }
            else
            {
                if (LODMedical.DeathInvolved == "Yes")
                {
                    notificationType = (byte)MemoType.ANGLodFindingsNLODDeath;
                }
                else
                {
                    notificationType = (byte)MemoType.ANGLodFindingsNLOD;
                }
            }
            if (!notificationType.HasValue)
            {
        ...
        private void ProcessOption(int lastStatus, IDaoFactory daoFactory)
        {
            ...
            options = from o in RuleAppliedOptions where o.Compo == "5" select o;
        ...
        {
            //ALOD.Data data = new ALOD.Data
            //LineOfDutyPostProcessingDao
        ...
        protected virtual bool AFRCHasDeterminationMemo(IDaoFactory daoFactory)
        {
            IMemoDao memoDao = daoFactory.GetMemoDao();
            int count = 0;

            if (FinalFindings == (short)Finding.In_Line_Of_Duty || FinalFindings == (short)Finding.Epts_Service_Aggravated || LodFindings.ElementAt(1).Description.Equals("In Line of Duty"))
            {
        ...
        protected virtual bool ANGHasDeterminationMemo(IDaoFactory daoFactory)
        {
            IMemoDao memoDao = daoFactory.GetMemoDao();
            int count = 0;
            ...
            if (FinalFindings == (short)Finding.In_Line_Of_Duty || FinalFindings == (short)Finding.Epts_Service_Aggravated || LodFindings.ElementAt(1).Description.Equals("In Line of Duty"))
            {
                ...
                // much later than the normal ILOD memo, which means that there exists legacy LOD cases that are ILOD with death involved
                // that had the normal ILOD memo generated.
                count = memoDao.GetByRefId(Id).Where(x => x.Deleted == false && (x.Template.Id == (int)MemoType.ANGLodFindingsILOD || x.Template.Id == (int)MemoType.ANGLodFindingsILODDeath)).ToList().Count;
            }
            else
            {
                if (LODMedical.DeathInvolved.Equals("Yes"))
                    count = memoDao.GetByRefId(Id).Where(x => x.Deleted == false && x.Template.Id == (int)MemoType.ANGLodFindingsNLODDeath).ToList().Count;
                else
                    count = memoDao.GetByRefId(Id).Where(x => x.Deleted == false && x.Template.Id == (int)MemoType.ANGLodFindingsNLOD).ToList().Count;
            }
        ...
        protected virtual bool HasPostCompletionDigitalSignatureBeenGenerated(IDaoFactory daoFactory)
        {
            ...
            //if (sigMetaDataDao.GetByWorkStatus(Id, Workflow, (int)LodWorkStatus_v2.Complete) == null)
            if (sigMetaDataDao.GetByWorkStatus(Id, Workflow, (int)LodWorkStatus.Complete) == null)
            {
                //if (sigMetaDataDao.GetByWorkStatus(Id, Workflow, (int)LodWorkStatus_v3.Complete_LODV3) != null)
                //{
                //    return true;
                //}
                //else
