...
{
    [Serializable]
    public class SC_BMT : SpecialCase
    {
        public SC_BMT()
        {
        }
        ...
        {
            get { return (int)DocumentViewType.BMT; }
        }
        ...
        public override Dictionary<String, PageAccessType> ReadSectionList(int role)
        {
            ...
            if ((CurrentStatusCode == (int)SpecCaseBMTStatusCode.Approved) || (CurrentStatusCode == (int)SpecCaseBMTStatusCode.Denied))
            {
                access = PageAccessType.ReadOnly;
            }

            //'Add all pages as readonly
            scAccessList.Add(BTSectionNames.BT_HQT_INIT.ToString(), access);
            scAccessList.Add(BTSectionNames.BT_BOARD_SG_REV.ToString(), access);
            scAccessList.Add(BTSectionNames.BT_HQT_FINAL_REV.ToString(), access);
            scAccessList.Add(BTSectionNames.BT_SENIOR_MED_REV.ToString(), access);
            scAccessList.Add(BTSectionNames.BT_APPROVED.ToString(), access);
            scAccessList.Add(BTSectionNames.BT_DENIED.ToString(), access);
            scAccessList.Add(BTSectionNames.BT_RLB.ToString(), access);

            //Modify access with user role
            switch (role)
            {
                case (int)UserGroups.AFRCHQTechnician:
                    if (CurrentStatusCode == (int)SpecCaseBMTStatusCode.InitiateCase)
                    {
                        scAccessList[BTSectionNames.BT_HQT_INIT.ToString()] = PageAccessType.ReadWrite;
                    }
                    if (CurrentStatusCode == (int)SpecCaseBMTStatusCode.FinalReview)
                    {
                        scAccessList[BTSectionNames.BT_HQT_FINAL_REV.ToString()] = PageAccessType.ReadWrite;
                    }
                    break;

                case (int)UserGroups.BoardMedical:
                    if (CurrentStatusCode == (int)SpecCaseBMTStatusCode.MedicalReview)
                    {
                        scAccessList[BTSectionNames.BT_BOARD_SG_REV.ToString()] = PageAccessType.ReadWrite;
                    }
                    break;

                case (int)UserGroups.SeniorMedicalReviewer:
                    if (CurrentStatusCode == (int)SpecCaseBMTStatusCode.SeniorMedicalReview)
                    {
                        scAccessList[BTSectionNames.BT_SENIOR_MED_REV.ToString()] = PageAccessType.ReadWrite;
                    }
                    break;
                    ...
                    case "memo":
                        ...
                        for (int i = 0; i < memos.Length; i++)
                        {
                            if (!String.IsNullOrEmpty(memos[i]))
                            {
                                ...
                                IList<Memorandum2> memolist = (from m in memoDao.GetByRefnModule(Id, (int)ModuleType.SpecCaseBMT) where m.Deleted == false && m.Template.Id == Convert.ToByte(memos[i]) select m).ToList<Memorandum2>();
                                if (memolist.Count > 0)
                                {
                                    oneExist = true;
                                }
        ...
        protected bool DetermineVisibilityForCanCompleteCaseRule(int expectedFinding)
        {
            ...
            {
                AddValidationItem(new ValidationItem("Board Medical Officer", "Decision", "Board Medical Officer must provide a finding in order to close out the BMT case."));
                return false;
            }
        ...
        private bool ValidateCanForwardCaseRuleForBoardMedicalOfficer()
        {
            if (!med_off_approved.HasValue)
            {
                AddValidationItem(new ValidationItem("Board Medical Officer", "Decision", "Must provide a Decision for the BMT case."));
                return false;
            }
        ...
        private bool ValidateCanForwardCaseRuleForSeniorMedicalReviewer()
        {
            ...
            {
                canForward = false;
                AddValidationItem(new ValidationItem("Senior Medical Reviewer", "Decision", "Must provide a Concur/Non-Concur Decision for the BMT case."));
            }
            else
            {
                ...
                {
                    canForward = false;
                    AddValidationItem(new ValidationItem("Senior Medical Reviewer", "Determination", "Must provide a Decision for the BMT case."));
                }
            }

            if (string.IsNullOrEmpty(SeniorMedicalReviewerComment))
            {
                AddValidationItem(new ValidationItem("Senior Medical Reviewer", "Decision Explanation", "Must provide a Decision Explanation for the BMT case."));
                canForward = false;
            }
