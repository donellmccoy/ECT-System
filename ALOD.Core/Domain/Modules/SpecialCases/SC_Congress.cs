...
{
    [Serializable]
    public class SC_Congress : SpecialCase
    {
        public SC_Congress()
        {
        }
        ...
        {
            get { return (int)DocumentViewType.Congress; }
        }
        ...
        public override Dictionary<String, PageAccessType> ReadSectionList(int role)
        {
            ...
            if ((CurrentStatusCode == (int)SpecCaseCongressStatusCode.Complete))
            {
            ...
            scAccessList.Add(CISectionNames.CI_HQT_INIT.ToString(), access);
            scAccessList.Add(CISectionNames.CI_BOARD_SG_REV.ToString(), access);
            scAccessList.Add(CISectionNames.CI_HQT_FINAL_REV.ToString(), access);
            scAccessList.Add(CISectionNames.CI_SENIOR_MED_REV.ToString(), access);
            scAccessList.Add(CISectionNames.CI_COMPLETE.ToString(), access);
            scAccessList.Add(CISectionNames.CI_RLB.ToString(), access);

            //Modify access with user role
            switch (role)
            {
                case (int)UserGroups.AFRCHQTechnician:
                    if (CurrentStatusCode == (int)SpecCaseCongressStatusCode.InitiateCase)
                    {
                        scAccessList[CISectionNames.CI_HQT_INIT.ToString()] = PageAccessType.ReadWrite;
                        scAccessList[CISectionNames.CI_RLB.ToString()] = PageAccessType.ReadWrite;
                    }
                    else
                    {
                        scAccessList[CISectionNames.CI_COMPLETE.ToString()] = PageAccessType.ReadWrite;
                    }
                    break;

                case (int)UserGroups.BoardMedical:
                    if (CurrentStatusCode == (int)SpecCaseCongressStatusCode.MedicalReview)
                    {
                        scAccessList[CISectionNames.CI_BOARD_SG_REV.ToString()] = PageAccessType.ReadWrite;
                        scAccessList[CISectionNames.CI_RLB.ToString()] = PageAccessType.ReadWrite;
                    }
                    break;

                case (int)UserGroups.SeniorMedicalReviewer:
                    if (CurrentStatusCode == (int)SpecCaseCongressStatusCode.SeniorMedicalReview)
                    {
                        scAccessList[CISectionNames.CI_SENIOR_MED_REV.ToString()] = PageAccessType.ReadWrite;
                        scAccessList[CISectionNames.CI_RLB.ToString()] = PageAccessType.ReadWrite;
                    }
                    break;
                    ...
                    case "memo":
                        ...
                        for (int i = 0; i < memos.Length; i++)
                        {
                            if (!String.IsNullOrEmpty(memos[i]))
                            {
                                ...
                                IList<Memorandum2> memolist = (from m in memoDao.GetByRefnModule(Id, (int)ModuleType.SpecCaseCongress) where m.Deleted == false && m.Template.Id == Convert.ToByte(memos[i]) select m).ToList<Memorandum2>();
                                if (memolist.Count > 0)
                                {
                                    oneExist = true;
                                }
