...
namespace ALOD.Core.Domain.Documents
{
    ...
    public class PdfTable
    {
        ...
        public void AddHtml(IList<PDFTableHTMLData> data)
        {
            string originalColor;
            bool originalBold;
            double originalHPos;

            foreach (PDFTableHTMLData d in data)
            {
                NextCell();

                originalColor = mDoc.Color.String;
                originalBold = mDoc.TextStyle.Bold;
                originalHPos = mDoc.HPos;

                if (!string.IsNullOrEmpty(d.Color))
                    mDoc.Color.String = d.Color;

                mDoc.TextStyle.Bold = d.IsBold;
                mDoc.HPos = d.HorizontalPosition;

                AddHtml(d.Text);

                mDoc.Color.String = originalColor;
                mDoc.TextStyle.Bold = originalBold;
                mDoc.HPos = originalHPos;
            }
        }
